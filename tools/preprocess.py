...
Usage:
    cd aboot && python3 tools/preprocess.py
    # Writes src/aboot.c
"""
import json
import re
...
# Paths relative to this script (aboot/tools/preprocess.py)
...
OUT_C      = ABOOT_DIR / "src" / "aboot.c"
TYPES_H    = Path("ghidra_types.h")  # relative include in output C (same dir as aboot.c)

# ELF load address mapping for aboot.bin
# LOAD segment: file offset 0x8000 → vaddr 0x8f600000
...
def vaddr_to_file_offset(vaddr: int) -> int:
    """Convert virtual address to file offset in aboot.bin."""
...
# Regex for Ghidra's RAM/Stack/Param variable names like:
#   uRam000901a4, cRam00083554, puRam000901a8, ppuRam000901a8, iStack_18, etc.
# Prefixes: u/i/c/b/s/d/q (basic), pu/pi/pc/pb/ps/pd/pq (pointer), ppu/ppi/ppc/ppb/pps/ppd/ppq (double pointer)
...
def collect_string_syms(all_c: str) -> set[str]:
    ...
    Handles Ghidra symbols like s_foo_8f123456 and s_<tag>_8f123456.
    Returns sanitized names (e.g. < → _lt_) to match what transform_source produces.
    """
    syms = set()
    # Match s_ prefix, any mix of word chars and <>/?, ending with _8hexdigits
...
def transform_source(c_source: str) -> str:
    ...
    # Match any s_... with address suffix that contains special chars
    ...
    def _is_unary_context(before_text):
        ...
        if last_char in '([{}=,;!~^|&<>+-*/':
            return True
        # ) can precede unary * only when it closes a cast expression like (uint), (uint *)
        # NOT when it closes a general expression like (iVar3 + 0x44) where * is multiplication.
        # Heuristic: cast content contains only word chars, spaces, and * (no operators/brackets)
        if last_char == ')':
...
def main():
    if not FUNCS_JSON.exists():
        print("Run ghidra_analyze.py first")
        sys.exit(1)
    ...
    # Also capture ram0x... style variables
    ...
    for m in re.finditer(r'\b(PTR_LAB_[0-9a-f]{8}(?:_[0-9]+_[0-9a-f]+)?)\b', all_c):
        # Pointer to label (may have suffix like _1_8f625134)
        misc_decls.append(f"extern uint *{m.group(1)};")
    ...
    # Note: string symbols with special chars are handled in string_externs
    for m in re.finditer(r'\b(_[0-9a-f]{8})\b', all_c):
        # Standalone address symbols
        misc_decls.append(f"extern uint {m.group(1)};")
    print(f"[*] Collected misc symbols in {time.time()-t0:.2f}s")

    # Collect string symbols (extern declarations for aboot.c, definitions go in strings.c)
    t0 = time.time()
    ...
    # Find void functions to upgrade (use simpler, faster patterns)
    void_funcs = set()
    ...
    upgrade_to_u4 = set()
    # Simpler patterns to find functions used as values
    simple_patterns = [
        r'= ((?:thunk_)?FUN_[0-9a-f]{8})\s*\(',  # assignment
        r'return ((?:thunk_)?FUN_[0-9a-f]{8})\s*\(',  # return
        r',\s*((?:thunk_)?FUN_[0-9a-f]{8})\s*\(',  # comma
        r'\)\s*((?:thunk_)?FUN_[0-9a-f]{8})\s*\(',  # after cast close paren
    ]
    ...
    # Assemble output
    lines = [
        "/* Auto-generated: aboot decompiled + preprocessed for arm-none-eabi-gcc */",
        f'#include "{TYPES_H}"',
        "",
    ...
    print(f"    Strings are in src/strings.c (regenerate with: make strings)")
