/* Auto-generated: fhprg_8021ff0.c */
/* Functions: 41 */
#include "globals.h"

/* 0x08021ff0 */
int FUN_08021ff0(param_1, param_2, param_3)
int param_1; int * param_2; int param_3;
{
  int iVar1;
  
  iVar1 = -1;
  if (((param_2 != (int *)0x0) && (param_3 != 0)) && (*param_2 <= param_1)) {
    do {
      iVar1 = param_3 + -1;
      if (iVar1 < 0) {
        return iVar1;
      }
      param_3 = iVar1;
    } while (param_1 < param_2[iVar1]);
  }
  return iVar1;
}




/* 0x08022018 */
undefined4 FUN_08022018(param_1, param_2, param_3, param_4)
uint param_1; uint param_2; undefined4 param_3; undefined4 param_4;
{
  int iVar1;
  undefined4 uVar2;
  undefined8 uVar3;
  
  iVar1 = FUN_0801a490(param_3,param_4);
  uVar3 = FUN_080063f0(1,0,0x3f - iVar1);
  uVar2 = FUN_0801a502((uint)uVar3 | param_1,(uint)((ulonglong)uVar3 >> 0x20) | param_2);
  FUN_080063f0(1,0,uVar2);
  return 0;
}




/* 0x08022048 */
uint FUN_08022048(param_1, param_2, param_3, param_4, param_5)
int param_1; int param_2; int param_3; undefined4 param_4; int param_5;
{
  short sVar1;
  int iVar2;
  uint uVar3;
  uint uVar4;
  int iVar5;
  int iVar6;
  int iVar7;
  uint uVar8;
  
  iVar6 = 0;
  iVar5 = 0;
  uVar8 = 0;
  FUN_0801242c(param_2,param_3,param_4,0);
  iVar7 = param_5;
  do {
    iVar7 = iVar7 + -1;
    sVar1 = *(short *)(param_1 + iVar7 * 2);
    if (iVar7 < 1) {
      if (sVar1 != 0x2e) {
        iVar7 = param_5;
      }
      break;
    }
  } while (sVar1 != 0x2e);
  if ((iVar7 < 9) && ((param_5 - iVar7) + -1 < 4)) {
    iVar2 = 0;
    do {
      uVar3 = (uint)*(ushort *)(param_1 + iVar2 * 2);
      if ((uVar3 - 0x61 < 0x1a) || (uVar3 - 0x41 < 0x1a)) {
        iVar6 = iVar6 + 1;
      }
      if ((*(char *)(param_3 + iVar2) == ' ') ||
         (uVar4 = (uint)*(ushort *)(param_2 + iVar2 * 2), uVar4 == 0x2e)) break;
      if (uVar4 != uVar3) {
        if (uVar3 - 0x61 < 0x1a) {
          uVar3 = uVar3 - 0x20;
        }
        if (uVar4 != uVar3) {
          return 0xff;
        }
        iVar5 = iVar5 + 1;
      }
      iVar2 = iVar2 + 1;
    } while (iVar2 < 8);
    if (0 < iVar5) {
      if (iVar6 != iVar5) {
        return 0xff;
      }
      uVar8 = 8;
    }
    iVar5 = 0;
    iVar6 = 0;
    while (iVar7 = iVar7 + 1, iVar7 < param_5) {
      uVar3 = (uint)*(ushort *)(param_1 + iVar7 * 2);
      if ((uVar3 - 0x61 < 0x1a) || (uVar3 - 0x41 < 0x1a)) {
        iVar6 = iVar6 + 1;
      }
      uVar4 = (uint)*(ushort *)(param_2 + iVar7 * 2);
      if ((uVar4 == 0) || (uVar4 == 0x2e)) break;
      if (uVar4 != uVar3) {
        if (uVar3 - 0x61 < 0x1a) {
          uVar3 = uVar3 - 0x20;
        }
        if (uVar4 != uVar3) {
          return 0xff;
        }
        iVar5 = iVar5 + 1;
      }
    }
    if (iVar5 < 1) {
      return uVar8;
    }
    if (iVar6 == iVar5) {
      return uVar8 | 0x10;
    }
  }
  return 0xff;
}




/* 0x0802212c */
undefined4 FUN_0802212c(param_1, param_2, param_3)
int param_1; uint param_2; undefined4 * param_3;
{
  uint uVar1;
  int iVar2;
  uint uVar3;
  
  uVar1 = 0;
  *param_3 = 0xffffffff;
  while( true ) {
    if (*(uint *)(param_1 + 0x10) <= uVar1) {
      return 5;
    }
    iVar2 = param_1 + uVar1 * 0x40;
    uVar3 = *(uint *)(iVar2 + 0x28);
    if (((uint)-*(int *)(iVar2 + 0x2c) < (uint)(uVar3 <= param_2)) &&
       ((uint)(uVar3 + *(uint *)(iVar2 + 0x30) <= param_2) <=
        -(*(int *)(iVar2 + 0x2c) +
         *(int *)(iVar2 + 0x34) + (uint)CARRY4(uVar3,*(uint *)(iVar2 + 0x30))))) break;
    uVar1 = uVar1 + 1;
  }
  *param_3 = *(undefined4 *)(iVar2 + 0x3c);
  return 0;
}




/* 0x08022178 */
int FUN_08022178(param_1, param_2, param_3, param_4, param_5, param_6)
int param_1; undefined4 param_2; int param_3; undefined4 param_4; uint param_5; uint param_6;
{
  longlong lVar1;
  longlong lVar2;
  uint uVar3;
  uint uVar4;
  int iVar5;
  uint uVar6;
  uint uVar7;
  uint uVar8;
  uint uVar9;
  
  param_1 = param_1 + param_3 * 4;
  uVar9 = *(uint *)(param_1 + -4);
  lVar1 = (ulonglong)uVar9 * (ulonglong)param_5;
  uVar8 = (uint)((ulonglong)lVar1 >> 0x20);
  lVar2 = (ulonglong)*(uint *)(param_1 + -8) * (ulonglong)param_6;
  uVar7 = (uint)((ulonglong)lVar2 >> 0x20);
  uVar4 = (uint)CARRY4((uint)lVar1,(uint)lVar2);
  uVar3 = uVar4 + uVar8;
  lVar1 = (ulonglong)param_6 * (ulonglong)uVar9;
  uVar6 = (uint)lVar1;
  uVar9 = uVar3 + uVar7;
  uVar4 = uVar9 + uVar6 >> 0x1e |
          ((uint)CARRY4(uVar4,uVar8) + (uint)CARRY4(uVar3,uVar7) + (uint)CARRY4(uVar9,uVar6)) * 4;
  uVar3 = (int)((ulonglong)lVar1 >> 0x20) * 4;
  iVar5 = uVar3 + uVar4;
  if (CARRY4(uVar3,uVar4)) {
    iVar5 = -1;
  }
  return iVar5;
}




/* 0x080221c8 */
uint FUN_080221c8(param_1, param_2)
int param_1; int param_2;
{
  byte *pbVar1;
  uint uVar2;
  int iVar3;
  
  uVar2 = 0;
  iVar3 = 0;
  do {
    pbVar1 = (byte *)(param_1 + iVar3);
    iVar3 = iVar3 + 1;
    uVar2 = (uVar2 << 7 | uVar2 >> 1) + (uint)*pbVar1 & 0xff;
  } while (iVar3 < 8);
  iVar3 = 0;
  do {
    pbVar1 = (byte *)(param_2 + iVar3);
    iVar3 = iVar3 + 1;
    uVar2 = (uVar2 << 7 | uVar2 >> 1) + (uint)*pbVar1 & 0xff;
  } while (iVar3 < 3);
  return uVar2;
}




/* 0x080221f8 */
void FUN_080221f8(param_1, param_2, param_3)
uint * param_1; int param_2; int param_3;
{
  undefined1 uVar1;
  uint uVar2;
  uint *puVar3;
  uint *puVar4;
  uint uVar5;
  uint uVar6;
  uint *puVar7;
  uint uVar8;
  uint uVar9;
  uint uVar10;
  uint uVar11;
  uint uVar12;
  uint uVar13;
  uint auStack_148 [60];
  uint local_58;
  uint local_54;
  uint local_50;
  uint local_4c;
  uint *local_48;
  uint local_44;
  uint local_40;
  int local_3c;
  uint local_38;
  uint local_34;
  uint *local_30;
  int local_2c;
  int local_28;
  
  local_30 = param_1;
  local_2c = param_2;
  local_28 = param_3;
  FUN_08006d14(param_3,0xf0);
  local_38 = 0;
  do {
    if (*local_30 <= local_38) {
      return;
    }
    local_34 = 0;
    FUN_08006d14(auStack_148,0xf0);
    uVar9 = 0;
    for (uVar5 = 0; uVar5 < local_30[1]; uVar5 = uVar5 + 1) {
      thunk_FUN_00221cd8(local_30[4],*(undefined4 *)(local_30[5] + local_38 * 4),uVar5,&local_58);
      uVar2 = local_34;
      uVar10 = local_4c;
      uVar6 = uVar9;
      if ((char)local_58 != '\0') {
        if (local_40 == 0) {
          puVar3 = (uint *)(local_28 + local_38 * 0x78);
          uVar6 = uVar9 + 1;
          uVar2 = uVar9;
        }
        else {
          if (((local_2c != 0) &&
              (uVar9 = local_30[6],
              local_50 == *(uint *)(uVar9 + 8) && local_4c == *(uint *)(uVar9 + 0xc))) &&
             (local_48 == *(uint **)(uVar9 + 0x10) && local_44 == *(uint *)(uVar9 + 0x14)))
          goto LAB_0802229c;
          puVar3 = auStack_148 + local_38 * 0x1e;
          local_34 = local_34 + 1;
        }
        puVar3 = puVar3 + uVar2 * 6;
        *puVar3 = local_50;
        puVar3[1] = uVar10;
        uVar9 = local_44;
        puVar3[2] = (uint)local_48;
        puVar3[3] = uVar9;
        if ((local_3c == 2) || (local_3c == 1)) {
          uVar1 = 1;
        }
        else {
          uVar1 = 0;
        }
        *(undefined1 *)(puVar3 + 4) = uVar1;
      }
LAB_0802229c:
      uVar9 = uVar6;
    }
    for (local_4c = 0; local_4c < local_34; local_4c = local_4c + 1) {
      for (local_54 = 0; local_54 < local_34; local_54 = local_54 + 1) {
        local_3c = local_38 * 0xf;
        puVar3 = auStack_148 + local_38 * 0x1e + local_54 * 6;
        if (puVar3[2] != 0 || puVar3[3] != 0) {
          for (uVar9 = 0; uVar9 < local_30[1]; uVar9 = uVar9 + 1) {
            puVar4 = (uint *)(local_28 + local_3c * 8 + uVar9 * 0x18);
            local_48 = puVar4;
            uVar5 = puVar4[2];
            uVar10 = puVar4[3];
            if (uVar5 != 0 || uVar10 != 0) {
              uVar11 = *puVar4;
              local_58 = puVar4[1];
              uVar2 = uVar11 + uVar5;
              local_40 = local_58 + uVar10 + CARRY4(uVar11,uVar5);
              uVar12 = *puVar3;
              local_50 = puVar3[1];
              uVar8 = puVar3[2];
              uVar6 = puVar3[3];
              uVar13 = uVar12 + uVar8;
              local_44 = local_50 + uVar6 + (uint)CARRY4(uVar12,uVar8);
              if (uVar2 == uVar13 && local_40 - local_44 == (uint)(uVar2 < uVar13)) {
                puVar4[2] = uVar5 - uVar8;
                puVar4[3] = (uVar10 - uVar6) - (uint)(uVar5 < uVar8);
              }
              else if ((local_50 < local_58 || local_58 - local_50 < (uint)(uVar12 <= uVar11)) ||
                      (local_40 < local_44 || local_44 - local_40 < (uint)(uVar2 <= uVar13))) {
                if ((uVar11 != uVar12 || local_58 != local_50) ||
                   (uVar10 < uVar6 || uVar6 - uVar10 < (uint)(uVar5 <= uVar8))) goto LAB_080223fe;
                *puVar4 = uVar11 + uVar8;
                puVar4[1] = local_58 + uVar6 + (uint)CARRY4(uVar11,uVar8);
                uVar5 = puVar4[2];
                uVar10 = puVar3[2];
                uVar2 = puVar3[3];
                puVar4[2] = uVar5 - uVar10;
                puVar4[3] = (puVar4[3] - uVar2) - (uint)(uVar5 < uVar10);
              }
              else {
                puVar7 = puVar4 + 6;
                FUN_08006bdc(puVar7,puVar4,0x18);
                uVar5 = *puVar3;
                uVar10 = puVar3[1];
                puVar4[2] = uVar5 - *puVar4;
                puVar4[3] = (uVar10 - puVar4[1]) - (uint)(uVar5 < *puVar4);
                uVar5 = *puVar3;
                uVar12 = puVar3[1];
                uVar10 = uVar5 - *puVar7;
                uVar2 = puVar3[2];
                uVar8 = puVar3[3];
                uVar6 = uVar10 + uVar2;
                uVar11 = puVar4[8];
                puVar4[8] = uVar11 - uVar6;
                puVar4[9] = (puVar4[9] -
                            (((uVar12 - puVar4[7]) - (uint)(uVar5 < *puVar7)) + uVar8 +
                            (uint)CARRY4(uVar10,uVar2))) - (uint)(uVar11 < uVar6);
                uVar5 = *puVar3;
                uVar6 = puVar3[1];
                uVar10 = puVar3[2];
                uVar2 = puVar3[3];
                *puVar7 = uVar5 + uVar10;
                puVar4[7] = uVar6 + uVar2 + CARRY4(uVar5,uVar10);
              }
              *puVar3 = 0;
              puVar3[1] = 0;
              puVar3[2] = 0;
              puVar3[3] = 0;
            }
LAB_080223fe:
          }
        }
      }
    }
    local_38 = local_38 + 1;
  } while( true );
}



/* WARNING: Globals starting with '_' overlap smaller symbols at the same address */

/* 0x08022440 */
int FUN_08022440()
{
  do {
  } while (false);
  return _DAT_004a3000 * 0x1e + (_DAT_004a3000 >> 1);
}




/* 0x0802245c */
uint FUN_0802245c(param_1, param_2, param_3)
byte * param_1; uint param_2; uint param_3;
{
  uint uVar1;
  
  uVar1 = 0;
  if ((param_1 != (byte *)0x0) && (param_2 != 0)) {
    param_3 = ~param_3;
    for (; uVar1 < param_2; uVar1 = uVar1 + 1) {
      param_3 = *(uint *)(&DAT_0803d71c + ((uint)*param_1 ^ param_3 & 0xff) * 4) ^ param_3 >> 8;
      param_1 = param_1 + 1;
    }
    uVar1 = ~param_3;
  }
  return uVar1;
}




/* 0x0802248c */
undefined4 FUN_0802248c(param_1, param_2, param_3, param_4)
undefined4 param_1; undefined4 param_2; undefined8 * param_3; uint * param_4;
{
  int iVar1;
  int iVar2;
  int iVar3;
  uint uVar4;
  uint uVar5;
  uint uVar6;
  uint uVar7;
  uint uVar8;
  longlong lVar9;
  undefined8 uVar10;
  longlong lVar11;
  
  iVar1 = FUN_0802efd6(param_1,param_2,1,param_4[8],param_4);
  if ((iVar1 != 0) && (iVar2 = FUN_08006898(iVar1,"EFI PART",8), iVar2 == 0)) {
    iVar2 = FUN_08006e04();
    FUN_08006e1a(0,iVar1 + 0x10);
    iVar3 = FUN_0802245c(iVar1,0x5c,0);
    FUN_08006e1a(iVar2,iVar1 + 0x10);
    if (iVar3 == iVar2) {
      FUN_0802264c(iVar1,0);
      lVar9 = FUN_08006e30(iVar1 + 0x18);
      if (lVar9 == CONCAT44(param_2,param_1)) {
        uVar10 = FUN_08006e30(iVar1 + 0x20);
        *param_3 = uVar10;
        uVar4 = FUN_08006e04(iVar1 + 0x50);
        if (uVar4 != 0) {
          uVar5 = FUN_08006e04(iVar1 + 0x54);
          if ((uVar5 != 0) && ((uVar5 & 0x7f) == 0)) {
            uVar5 = uVar5 * uVar4;
            uVar6 = (uVar5 - 1) / *param_4;
            iVar2 = iVar1 + 0x48;
            lVar9 = FUN_08006e30();
            param_4[1] = uVar4;
            param_4[7] = 0;
            uVar4 = FUN_08006e04(iVar1 + 0x54);
            param_4[6] = uVar4;
            uVar10 = FUN_08006e30(iVar2);
            *(undefined8 *)(param_4 + 2) = uVar10;
            lVar11 = FUN_08006e30(iVar2);
            lVar11 = lVar11 + CONCAT44((uVar6 != 0xffffffff) - 1,uVar6);
            uVar7 = (uint)lVar11;
            uVar6 = (uint)((ulonglong)lVar11 >> 0x20);
            *(longlong *)(param_4 + 4) = lVar11;
            uVar8 = *(uint *)(param_4[8] + 0xc);
            uVar10 = FUN_08006e30(iVar2);
            uVar4 = (uint)((ulonglong)uVar10 >> 0x20);
            if ((uVar4 < uVar6 || uVar6 - uVar4 < (uint)((uint)uVar10 <= uVar7)) &&
               (-uVar6 < (uint)(uVar7 <= uVar8))) {
              iVar1 = FUN_08006e04(iVar1 + 0x58,uVar8 - uVar7);
              iVar2 = 0;
              while( true ) {
                if ((uVar5 == 0) ||
                   (iVar3 = FUN_0802efd6((int)lVar9,(int)((ulonglong)lVar9 >> 0x20),
                                         (param_4[4] - (int)lVar9) + 1,param_4[8]), iVar3 == 0))
                break;
                lVar9 = lVar9 + 1;
                uVar4 = *param_4;
                if (uVar5 <= *param_4) {
                  uVar4 = uVar5;
                }
                iVar2 = FUN_0802245c(iVar3,uVar4,iVar2);
                uVar5 = uVar5 - uVar4;
              }
              if (iVar2 != iVar1) {
                return 0xffffffff;
              }
              return 0;
            }
          }
        }
      }
    }
  }
  return 0xffffffff;
}




/* 0x080225f8 */
int FUN_080225f8(param_1)
uint * param_1;
{
  int iVar1;
  uint uVar2;
  uint uVar3;
  uint uVar4;
  uint uVar5;
  
  iVar1 = 0;
  if ((param_1[1] != 0) &&
     (iVar1 = FUN_0802efd6(param_1[2],param_1[3],(param_1[4] - param_1[2]) + 1,param_1[8]),
     iVar1 != 0)) {
    iVar1 = iVar1 + param_1[7];
    param_1[1] = param_1[1] - 1;
    uVar2 = param_1[6] + param_1[7];
    param_1[7] = uVar2;
    uVar3 = *param_1;
    if (uVar3 <= uVar2) {
      uVar4 = uVar2 / uVar3;
      uVar5 = param_1[2];
      param_1[7] = uVar2 - uVar3 * uVar4;
      param_1[2] = uVar4 + uVar5;
      param_1[3] = param_1[3] + (uint)CARRY4(uVar4,uVar5);
    }
  }
  return iVar1;
}




/* 0x0802264c */
void FUN_0802264c()
{
  return;
}




/* 0x08022650 */
undefined4 handle_benchmark()
{
  int iVar1;
  uint uVar2;
  uint uVar3;
  uint uVar4;
  uint uVar5;
  int iVar6;
  int iVar7;
  int iVar8;
  undefined4 uVar9;
  uint uVar10;
  uint uVar11;
  uint uVar12;
  uint uVar13;
  bool bVar14;
  undefined8 uVar15;
  uint local_260;
  undefined1 auStack_24c [4];
  byte local_248 [512];
  undefined1 auStack_48 [32];
  int local_28;
  
  iVar1 = DAT_0804cd58;
  local_28 = DAT_0804cd58;
  FUN_08006d14(local_248,0x200);
  FUN_08006d14(auStack_48,0x20);
  uVar5 = DAT_0805845c;
  uVar4 = DAT_08058458;
  uVar3 = DAT_08055fbc;
  uVar2 = DAT_08055fb8;
  uVar9 = DAT_08054e90;
  uVar10 = 0x35a0;
  while (iVar8 = FUN_08038db4(&DAT_08055ea0,uVar10), iVar8 == 2) {
    iVar8 = FUN_0803906c(&DAT_08055ea0,"TestWritePerformance");
    if ((iVar8 != 0) && (FUN_08038d38(&DAT_08055ea0,local_248,0x200,0), local_248[0] == 0x31)) {
      firehose_log("Going to test writes for lengths %llu to %llu for %llu trials. This can take a while."
                   ,uVar2,uVar4,uVar5,uVar2,uVar3,1000,0);
      FUN_08038206(&DAT_08058430,0);
      uVar15 = FUN_080060c0(uVar2,uVar3,uVar4,uVar5);
      FUN_08038014(&DAT_08058430,uVar9,0,0,uVar15);
      uVar10 = uVar4;
      for (uVar12 = uVar5; uVar12 < uVar3 || uVar3 - uVar12 < (uint)(uVar10 <= uVar2);
          uVar12 = uVar12 * 2 + (uint)bVar14) {
        iVar8 = FUN_08022440(0,uVar2 - uVar10);
        uVar13 = 0;
        uVar11 = 0;
        while( true ) {
          if (uVar11 != 0 || uVar11 < (999 < uVar13)) break;
          uVar15 = FUN_080060c0(uVar10,uVar12,uVar4,uVar5);
          iVar6 = mmc_write_wrapper(&DAT_08058430,uVar9,0,0,uVar15,auStack_24c);
          if (iVar6 != 1) {
            firehose_log("Failure in write during perf testing");
          }
          bVar14 = 0xfffffffe < uVar13;
          uVar13 = uVar13 + 1;
          uVar11 = uVar11 + bVar14;
        }
        iVar6 = FUN_08022440(uVar13 - 1000,uVar11 - (999 >= uVar13));
        uVar15 = FUN_080060c0(iVar6 - iVar8,iVar6 - iVar8 >> 0x1f,uVar13,uVar11);
        firehose_log("Writing %llu bytes %d us",(int)((ulonglong)uVar15 >> 0x20),uVar10,uVar12,
                     (int)uVar15);
        bVar14 = CARRY4(uVar10,uVar10);
        uVar10 = uVar10 * 2;
      }
    }
    iVar8 = FUN_0803906c(&DAT_08055ea0,"TestReadPerformance");
    if (iVar8 == 0) {
      uVar15 = FUN_0803906c(&DAT_08055ea0,"TestDigestPerformance");
      uVar10 = (uint)((ulonglong)uVar15 >> 0x20);
      if ((int)uVar15 != 0) {
        FUN_08038d38(&DAT_08055ea0,local_248,0x200,0);
        uVar10 = (uint)local_248[0];
        if (uVar10 == 0x31) {
          uVar12 = uVar4;
          for (local_260 = uVar5; uVar10 = uVar2 - uVar12,
              local_260 < uVar3 || uVar3 - local_260 < (uint)(uVar12 <= uVar2);
              local_260 = local_260 * 2 + (uint)bVar14) {
            iVar8 = 0;
            uVar10 = 0;
            for (uVar11 = 0; uVar11 == 0 && (999 < uVar10) <= uVar11; uVar11 = uVar11 + bVar14) {
              iVar6 = FUN_08022440(uVar11 - (999 >= uVar10));
              FUN_0800d59c(uVar9,uVar12,auStack_48);
              iVar7 = FUN_08022440();
              bVar14 = 0xfffffffe < uVar10;
              uVar10 = uVar10 + 1;
              iVar8 = iVar8 + (iVar7 - iVar6);
            }
            uVar15 = FUN_080060c0(iVar8,iVar8 >> 0x1f,uVar10,uVar11);
            firehose_log("Hashing %llu bytes %d us",(int)((ulonglong)uVar15 >> 0x20),uVar12,
                         local_260,(int)uVar15);
            bVar14 = CARRY4(uVar12,uVar12);
            uVar12 = uVar12 * 2;
          }
        }
      }
    }
    else {
      FUN_08038d38(&DAT_08055ea0,local_248,0x200,0);
      uVar10 = (uint)local_248[0];
      if (uVar10 == 0x31) {
        firehose_log("Going to test reads for lengths %llu to %llu for %llu trials. This can take a while."
                     ,uVar2,uVar4,uVar5,uVar2,uVar3,1000,0);
        FUN_08038206(&DAT_08058430,0);
        uVar12 = uVar4;
        for (uVar11 = uVar5; uVar10 = uVar2 - uVar12,
            uVar11 < uVar3 || uVar3 - uVar11 < (uint)(uVar12 <= uVar2);
            uVar11 = uVar11 * 2 + (uint)bVar14) {
          iVar8 = FUN_08022440();
          uVar10 = 0;
          uVar13 = 0;
          while( true ) {
            if (uVar13 != 0 || uVar13 < (999 < uVar10)) break;
            uVar15 = FUN_080060c0(uVar12,uVar11,uVar4,uVar5);
            iVar6 = FUN_08038014(&DAT_08058430,uVar9,0,0,uVar15);
            if (iVar6 != 1) {
              firehose_log("Failure in read during perf testing");
            }
            bVar14 = 0xfffffffe < uVar10;
            uVar10 = uVar10 + 1;
            uVar13 = uVar13 + bVar14;
          }
          iVar6 = FUN_08022440(uVar10 - 1000,uVar13 - (999 >= uVar10));
          uVar15 = FUN_080060c0(iVar6 - iVar8,iVar6 - iVar8 >> 0x1f,uVar10,uVar13);
          firehose_log("Reading %llu bytes %d us",(int)((ulonglong)uVar15 >> 0x20),uVar12,uVar11,
                       (int)uVar15);
          bVar14 = CARRY4(uVar12,uVar12);
          uVar12 = uVar12 * 2;
        }
      }
    }
  }
  uVar9 = handle_response(1);
  if (local_28 != iVar1) {
    FUN_08010960();
  }
  return uVar9;
}




/* 0x08022a00 */
undefined4 handle_configure()
{
  ulonglong uVar1;
  char *pcVar2;
  int iVar3;
  undefined4 uVar4;
  longlong lVar5;
  ulonglong uVar6;
  char local_26c [4];
  undefined4 local_268;
  undefined4 uStack_264;
  char *local_260;
  undefined4 local_25c;
  undefined4 uStack_258;
  char *local_254;
  undefined4 local_250;
  undefined4 uStack_24c;
  char *local_248;
  undefined4 uStack_244;
  undefined4 local_240;
  char *pcStack_23c;
  undefined4 local_238;
  undefined *puStack_234;
  int local_230;
  char local_22c [512];
  int local_2c;
  
  local_230 = DAT_0804cd58;
  local_2c = DAT_0804cd58;
  FUN_08006d14(local_22c,0x200);
  lVar5 = CONCAT44(DAT_08055fbc,DAT_08055fb8);
  uVar1 = CONCAT44(DAT_08057ffc,DAT_08057ff8);
  uVar4 = 1;
  DAT_08058480 = 0;
  DAT_08058484 = 0;
  DAT_08058481 = 0;
  DAT_08058482 = 0;
LAB_08022be2:
  DAT_08055fbc = (undefined4)((ulonglong)lVar5 >> 0x20);
  DAT_08055fb8 = (undefined4)lVar5;
  DAT_08057ffc = (undefined4)(uVar1 >> 0x20);
  DAT_08057ff8 = (undefined4)uVar1;
  iVar3 = FUN_08038db4(&DAT_08055ea0);
  if (iVar3 != 2) {
    firehose_log("logbuf@0x%08X fh@0x%08X",&DAT_08058028,&DAT_08054e90);
    local_238 = 0x73;
    puStack_234 = &DAT_08022e28;
    local_240 = 1;
    pcStack_23c = "TargetName";
    local_250 = 100;
    uStack_24c = 0x1000;
    local_248 = "Version";
    uStack_244 = 100;
    local_260 = "MaxPayloadSizeToTargetInBytesSupported";
    local_254 = "MaxXMLSizeInBytes";
    local_25c = 100;
    uStack_258 = 0x100000;
    local_268 = DAT_08055fb8;
    uStack_264 = DAT_08055fbc;
    uVar4 = FUN_08037084(uVar4,8,"MinVersionSupported",100,1,"MemoryName",0x73,&DAT_08022ea0,
                         "MaxPayloadSizeFromTargetInBytes",100,0x1000,
                         "MaxPayloadSizeToTargetInBytes",0x74);
    if (local_2c != local_230) {
      FUN_08010960();
    }
    return uVar4;
  }
  iVar3 = FUN_0803906c(&DAT_08055ea0,"MaxPayloadSizeToTargetInBytes");
  if (iVar3 == 0) {
    iVar3 = FUN_0803906c(&DAT_08055ea0,"MaxDigestTableSizeInBytes");
    if (iVar3 == 0) {
      iVar3 = FUN_0803906c(&DAT_08055ea0,"ZlpAwareHost");
      if (iVar3 == 0) {
        iVar3 = FUN_0803906c(&DAT_08055ea0,"SkipWrite");
        if (iVar3 == 0) {
          iVar3 = FUN_0803906c(&DAT_08055ea0,"AckRawData");
          if (iVar3 == 0) {
            iVar3 = FUN_0803906c(&DAT_08055ea0,"AckRawDataEveryNumPackets");
            if (iVar3 == 0) {
              iVar3 = FUN_0803906c(&DAT_08055ea0,"AlwaysValidate");
              if (iVar3 == 0) {
                iVar3 = FUN_0803906c(&DAT_08055ea0,"Verbose");
                uVar1 = CONCAT44(DAT_08057ffc,DAT_08057ff8);
                lVar5 = CONCAT44(DAT_08055fbc,DAT_08055fb8);
                if (iVar3 != 0) {
                  FUN_08038d38(&DAT_08055ea0,local_22c,0x200,0);
                  uVar1 = CONCAT44(DAT_08057ffc,DAT_08057ff8);
                  lVar5 = CONCAT44(DAT_08055fbc,DAT_08055fb8);
                  if (local_22c[0] == '1') {
                    DAT_08058482 = 1;
                    uVar1 = CONCAT44(DAT_08057ffc,DAT_08057ff8);
                    lVar5 = CONCAT44(DAT_08055fbc,DAT_08055fb8);
                  }
                }
              }
              else {
                FUN_08038d38(&DAT_08055ea0,local_22c,0x200,0);
                uVar1 = CONCAT44(DAT_08057ffc,DAT_08057ff8);
                lVar5 = CONCAT44(DAT_08055fbc,DAT_08055fb8);
                if (local_22c[0] == '1') {
                  DAT_08058481 = 1;
                  uVar1 = CONCAT44(DAT_08057ffc,DAT_08057ff8);
                  lVar5 = CONCAT44(DAT_08055fbc,DAT_08055fb8);
                }
              }
            }
            else {
              FUN_08038d38(&DAT_08055ea0,local_22c,0x200,0);
              DAT_08058484 = FUN_0803823e(local_22c,local_26c);
              uVar1 = CONCAT44(DAT_08057ffc,DAT_08057ff8);
              lVar5 = CONCAT44(DAT_08055fbc,DAT_08055fb8);
              if (local_26c[0] == '\0') {
                DAT_08058484 = 0;
                pcVar2 = "Failed to get ack_raw_data_every";
                goto LAB_08022b90;
              }
            }
          }
          else {
            FUN_08038d38(&DAT_08055ea0,local_22c,0x200,0);
            uVar1 = CONCAT44(DAT_08057ffc,DAT_08057ff8);
            lVar5 = CONCAT44(DAT_08055fbc,DAT_08055fb8);
            if (local_22c[0] == '1') {
              DAT_08058484 = 1;
              uVar1 = CONCAT44(DAT_08057ffc,DAT_08057ff8);
              lVar5 = CONCAT44(DAT_08055fbc,DAT_08055fb8);
            }
          }
        }
        else {
          FUN_08038d38(&DAT_08055ea0,local_22c,0x200,0);
          uVar1 = CONCAT44(DAT_08057ffc,DAT_08057ff8);
          lVar5 = CONCAT44(DAT_08055fbc,DAT_08055fb8);
          if (local_22c[0] == '1') {
            DAT_08058480 = 1;
            firehose_log("skipping write");
            uVar1 = CONCAT44(DAT_08057ffc,DAT_08057ff8);
            lVar5 = CONCAT44(DAT_08055fbc,DAT_08055fb8);
          }
        }
      }
      else {
        FUN_08038d38(&DAT_08055ea0,local_22c,0x200,0);
        uVar1 = CONCAT44(DAT_08057ffc,DAT_08057ff8);
        lVar5 = CONCAT44(DAT_08055fbc,DAT_08055fb8);
        if (local_22c[0] == '0') {
          FUN_0802f838(0);
          uVar1 = CONCAT44(DAT_08057ffc,DAT_08057ff8);
          lVar5 = CONCAT44(DAT_08055fbc,DAT_08055fb8);
        }
      }
      goto LAB_08022be2;
    }
    FUN_08038d38(&DAT_08055ea0,local_22c,0x200,0);
    uVar6 = FUN_0803823e(local_22c,local_26c);
    lVar5 = CONCAT44(DAT_08055fbc,DAT_08055fb8);
    uVar1 = CONCAT44(DAT_08057ffc,DAT_08057ff8);
    if (local_26c[0] == '\0') {
      pcVar2 = "Failed to get max hash table size";
      goto LAB_08022b90;
    }
    if (uVar6 == 0) goto LAB_08022be2;
    if ((uint)-(int)(uVar6 >> 0x20) < (uint)((uint)uVar6 < 0x2001)) {
      uVar1 = uVar6 >> 5;
      lVar5 = CONCAT44(DAT_08055fbc,DAT_08055fb8);
      goto LAB_08022be2;
    }
    pcVar2 = "Host\'s hash buffer size is too large";
  }
  else {
    FUN_08038d38(&DAT_08055ea0,local_22c,0x200,0);
    lVar5 = FUN_0803823e(local_22c,local_26c);
    uVar1 = CONCAT44(DAT_08057ffc,DAT_08057ff8);
    if (local_26c[0] == '\0') {
      pcVar2 = "Failed to get max payload size";
    }
    else if (lVar5 == 0) {
      pcVar2 = "Host\'s payload to target size is zero";
    }
    else {
      if ((uint)-(int)((ulonglong)lVar5 >> 0x20) < (uint)((uint)lVar5 < 0x100001))
      goto LAB_08022be2;
      pcVar2 = "Host\'s payload to target size is too large";
    }
  }
LAB_08022b90:
  firehose_log(pcVar2);
  uVar4 = 0;
  uVar1 = CONCAT44(DAT_08057ffc,DAT_08057ff8);
  lVar5 = CONCAT44(DAT_08055fbc,DAT_08055fb8);
  goto LAB_08022be2;
}




/* 0x08022ec8 */
undefined4 handle_erase()
{
  bool bVar1;
  int iVar2;
  int iVar3;
  uint uVar4;
  undefined4 uVar5;
  char local_220 [4];
  undefined1 auStack_21c [512];
  int local_1c;
  
  iVar2 = DAT_0804cd58;
  local_1c = DAT_0804cd58;
  FUN_08006d14(auStack_21c,0x200);
  bVar1 = true;
  uVar4 = 0xffffffff;
  while (iVar3 = FUN_08038db4(&DAT_08055ea0), iVar3 == 2) {
    iVar3 = FUN_0803906c(&DAT_08055ea0,"storagedrive");
    if (iVar3 != 0) {
      FUN_08038d38(&DAT_08055ea0,auStack_21c,0x200,0);
      uVar4 = FUN_0803823e(auStack_21c,local_220);
      if (local_220[0] == '\0') {
        firehose_log("Failed to get storage drive number");
        bVar1 = false;
      }
    }
  }
  if (bVar1) {
    if (uVar4 == 0xffffffff) {
      firehose_log("No storage drive number");
    }
    else {
      iVar3 = FUN_08038206(&DAT_08058430,uVar4 & 0xff);
      if (iVar3 != 0) {
        iVar3 = FUN_08037c56(&DAT_08058430);
        handle_response(iVar3 == 1);
        if (iVar3 == 1) {
          uVar5 = 0;
        }
        else {
          uVar5 = 3;
        }
        goto LAB_08022f50;
      }
      firehose_log("Invalid physical partition %d",uVar4);
    }
  }
  handle_response(0);
  uVar5 = 1;
LAB_08022f50:
  if (local_1c != iVar2) {
    FUN_08010960();
  }
  return uVar5;
}




/* 0x08022ffc */
undefined4 handle_firmwarewrite()
{
  bool bVar1;
  int iVar2;
  char *pcVar3;
  int iVar4;
  undefined4 extraout_r1;
  undefined4 extraout_r1_00;
  undefined4 extraout_r1_01;
  undefined4 extraout_r1_02;
  uint uVar5;
  uint uVar6;
  undefined4 uVar7;
  longlong lVar8;
  longlong lVar9;
  undefined8 uVar10;
  uint local_240;
  uint uStack_23c;
  char local_238 [4];
  undefined4 local_234;
  undefined1 auStack_230 [4];
  undefined1 auStack_22c [512];
  int local_2c;
  
  iVar2 = DAT_0804cd58;
  local_2c = DAT_0804cd58;
  lVar8 = 0;
  local_240 = 0;
  uStack_23c = 0;
  lVar9 = 0;
  FUN_08006d14(auStack_22c,0x200);
  bVar1 = true;
  while (uVar7 = (undefined4)((ulonglong)lVar9 >> 0x20), uVar10 = FUN_08038db4(&DAT_08055ea0),
        (int)uVar10 == 2) {
    iVar4 = FUN_0803906c(&DAT_08055ea0,"SECTOR_SIZE_IN_BYTES");
    if (iVar4 != 0) {
      FUN_08038d38(&DAT_08055ea0,auStack_22c,0x200,0);
      lVar8 = FUN_0803823e(auStack_22c,local_238);
      if (local_238[0] == '\0') {
        firehose_log("Failed to get Sector size value");
        bVar1 = false;
      }
    }
    iVar4 = FUN_0803906c(&DAT_08055ea0,"num_partition_sectors");
    if (iVar4 != 0) {
      FUN_08038d38(&DAT_08055ea0,auStack_22c,0x200,0);
      lVar9 = FUN_0803823e(auStack_22c,local_238);
      if (local_238[0] == '\0') {
        firehose_log("Failed to get Number of sectors to write");
        bVar1 = false;
      }
    }
  }
  if (bVar1) {
    firehose_log("firmwarewrite cmd: Sector size %llu",(int)((ulonglong)uVar10 >> 0x20),(int)lVar8,
                 (int)((ulonglong)lVar8 >> 0x20));
    firehose_log("firmwarewrite cmd: Number of Sectors %llu",extraout_r1,(int)lVar9,uVar7);
    if (lVar8 != CONCAT44(DAT_0805845c,DAT_08058458)) {
      firehose_log("Disk sector size does not match xml\n");
      pcVar3 = "Disk sector size :%llu XML gave:%llu";
      uVar7 = extraout_r1_00;
      uVar5 = DAT_08058458;
      uVar6 = DAT_0805845c;
LAB_0802310c:
      firehose_log(pcVar3,uVar7,uVar5,uVar6);
      goto LAB_080230ce;
    }
    lVar8 = lVar8 * lVar9;
    uVar5 = (uint)lVar8;
    uVar6 = (uint)((ulonglong)lVar8 >> 0x20);
    firehose_log("Bytes to read from channel: %llu");
    if (lVar8 == 0) {
      firehose_log("Firmware Binary must be non-zero in size. Check binary");
      goto LAB_080230ce;
    }
    if (DAT_08055fbc <= uVar6 && (uint)(uVar5 <= DAT_08055fb8) <= DAT_08055fbc - uVar6) {
      pcVar3 = "Bytes to read > buffer capacity of %llubytes";
      uVar7 = extraout_r1_01;
      uVar5 = DAT_08055fb8;
      uVar6 = DAT_08055fbc;
      goto LAB_0802310c;
    }
    FUN_08037084(1,1,"rawmode",0x73,&DAT_08023360);
    iVar4 = FUN_08021ca4(&local_234,extraout_r1_02,uVar5,uVar6,&local_240);
    if (iVar4 == 0) {
      if (uVar5 != local_240 || (uVar6 ^ uStack_23c) != 0) {
        firehose_log("Read Error.Bytes expected:%llu,read:%llu",uVar6 ^ uStack_23c,uVar5,uVar6,
                     local_240,uStack_23c);
        goto LAB_080231a4;
      }
      if (bVar1) {
        firehose_log("Channel read %llu bytes");
      }
      iVar4 = FUN_08037cb0(&DAT_08058430,local_234,(int)lVar9,uVar7,auStack_230);
      if (iVar4 == 0) goto LAB_080231a4;
      iVar4 = 1;
    }
    else {
      firehose_log("Failed to read full FW binary. Read %llu Bytes");
LAB_080231a4:
      iVar4 = 0;
    }
    FUN_08037084(iVar4,1,"rawmode",0x73,"false");
    if (iVar4 != 0) {
      uVar7 = 0;
      goto LAB_08023190;
    }
  }
  else {
LAB_080230ce:
    handle_response(0);
  }
  uVar7 = 1;
LAB_08023190:
  if (local_2c != iVar2) {
    FUN_08010960();
  }
  return uVar7;
}




/* 0x080233ec */
undefined4 FUN_080233ec(param_1)
int param_1;
{
  longlong lVar1;
  bool bVar2;
  ulonglong uVar3;
  ulonglong uVar4;
  ulonglong uVar5;
  ulonglong uVar6;
  longlong lVar7;
  longlong lVar8;
  int iVar9;
  char *pcVar10;
  undefined4 extraout_r1;
  undefined4 uVar11;
  int iVar12;
  ulonglong uVar13;
  undefined8 uVar14;
  uint uVar15;
  undefined1 auStack_3b0 [72];
  undefined1 auStack_368 [64];
  undefined1 auStack_328 [4];
  uint local_324;
  undefined8 local_320;
  char local_318 [4];
  uint local_314;
  undefined4 local_310;
  undefined4 local_30c;
  undefined1 *local_308;
  undefined1 *puStack_304;
  undefined8 local_300;
  int local_2f8;
  undefined4 local_2f4;
  undefined4 uStack_2f0;
  undefined4 local_2ec;
  undefined4 uStack_2e8;
  undefined *local_2e4;
  uint local_2e0;
  uint uStack_2dc;
  int local_2d8;
  undefined1 auStack_2d4 [512];
  char acStack_d4 [72];
  undefined1 auStack_8c [64];
  undefined1 auStack_4c [32];
  int local_2c;
  int local_28;
  
  local_2f8 = DAT_0804cd58;
  local_2c = DAT_0804cd58;
  local_28 = param_1;
  FUN_08006d14(auStack_2d4,0x200);
  local_324 = 0x200;
  uVar13 = 0;
  bVar2 = true;
  _GHIDRA_FIELD(local_320, 0, uint) = 0;
  _GHIDRA_FIELD(local_320, 4, uint) = 0;
  uVar15 = 0xffffffff;
  FUN_08006d14(auStack_8c,0x40);
  iVar12 = 0;
  FUN_08006d14(auStack_4c,0x20);
  FUN_08006d14(acStack_d4,0x48);
  local_314 = 0;
  local_30c = 0;
  local_2e0 = 0x4000;
  uStack_2dc = 0;
LAB_080234ee:
  while( true ) {
    iVar9 = FUN_08038db4(&DAT_08055ea0);
    if (iVar9 != 2) break;
    iVar9 = FUN_0803906c(&DAT_08055ea0,"start_sector");
    if (iVar9 == 0) {
      iVar9 = FUN_0803906c(&DAT_08055ea0,"num_partition_sectors");
      if (iVar9 == 0) {
        iVar9 = FUN_0803906c(&DAT_08055ea0,"physical_partition_number");
        if (iVar9 == 0) goto LAB_080234c0;
        FUN_08038d38(&DAT_08055ea0,auStack_2d4,0x200,0);
        uVar15 = FUN_0803823e(auStack_2d4,local_318);
        if (local_318[0] == '\0') {
          pcVar10 = "Failed to get physical_partition_number";
          goto LAB_080234e8;
        }
      }
      else {
        FUN_08038d38(&DAT_08055ea0,auStack_2d4,0x200,0);
        uVar13 = FUN_0803823e(auStack_2d4,local_318);
        if (local_318[0] == '\0') {
          pcVar10 = "Failed to get num_partition_sectors";
          goto LAB_080234e8;
        }
      }
    }
    else {
      FUN_08038d38(&DAT_08055ea0,auStack_8c,0x40,0);
    }
  }
  if (bVar2) {
    if (uVar15 != 0xffffffff) {
      if (local_324 == DAT_08058458 && DAT_0805845c == (int)local_324 >> 0x1f) {
        local_300 = FUN_080060c0(DAT_08055fb8,DAT_08055fbc,local_324,(int)local_324 >> 0x1f);
        local_2e4 = &DAT_08058430;
        uVar14 = FUN_08038206(&DAT_08058430,uVar15 & 0xff);
        if ((int)uVar14 == 0) {
          firehose_log("Invalid physical partition %d",uVar15);
        }
        else {
          iVar9 = FUN_08028c10(auStack_8c,(int)((ulonglong)uVar14 >> 0x20),0x40,0,&local_320);
          if (iVar9 != 0) {
            if (local_28 == 0) {
              FUN_080364e4(auStack_3b0);
            }
            local_310 = (&DAT_08054e90)[1 - DAT_08055fd4];
            local_308 = auStack_328;
            puStack_304 = auStack_368;
            local_2f4 = _GHIDRA_FIELD(local_320, 4, uint);
            uStack_2f0 = (undefined4)local_320;
            uVar5 = uVar13;
            uVar6 = local_300;
            lVar7 = CONCAT44(_GHIDRA_FIELD(local_320, 4, uint),(undefined4)local_320);
            lVar8 = 0;
            while( true ) {
              _GHIDRA_FIELD(local_320, 4, uint) = (undefined4)((ulonglong)lVar7 >> 0x20);
              _GHIDRA_FIELD(local_320, 0, uint) = (undefined4)lVar7;
              _GHIDRA_FIELD(local_300, 4, uint) = (uint)(uVar6 >> 0x20);
              _GHIDRA_FIELD(local_300, 0, uint) = (uint)uVar6;
              local_2ec = (undefined4)(uVar5 >> 0x20);
              uStack_2e8 = (undefined4)uVar5;
              uVar15 = (uint)(uVar13 >> 0x20);
              local_300 = uVar6;
              if (uVar13 == 0) break;
              if (uVar15 <= _GHIDRA_FIELD(local_300, 4, uint) &&
                  (uint)((uint)local_300 <= (uint)uVar13) <= uVar15 - _GHIDRA_FIELD(local_300, 4, uint)) {
                uVar6 = uVar13;
              }
              lVar1 = (uVar6 & 0xffffffff) * (ulonglong)local_324;
              uVar11 = (undefined4)lVar1;
              local_2d8 = (int)uVar6 * ((int)local_324 >> 0x1f) +
                          (int)(uVar6 >> 0x20) * local_324 + (int)((ulonglong)lVar1 >> 0x20);
              iVar12 = FUN_08038014(local_2e4,local_310,(undefined4)local_320,_GHIDRA_FIELD(local_320, 4, uint),uVar6)
              ;
              local_320 = lVar7;
              if (iVar12 == 0) {
                pcVar10 = "emmc read failed";
                goto LAB_080236a2;
              }
              local_320 = uVar6 + local_320;
              if (local_28 == 0) {
                FUN_08036ee8(auStack_3b0,puStack_304,local_308,local_310,uVar11);
                uVar14 = CONCAT44(extraout_r1,local_30c);
              }
              else {
                uVar14 = FUN_08019a70(local_310,(int)local_320,uVar11,local_2d8,local_30c);
              }
              local_30c = (undefined4)uVar14;
              lVar1 = uVar13 - uVar6;
              uVar4 = uVar13 - uVar6;
              uVar3 = uVar13 - uVar6;
              lVar8 = uVar6 + lVar8;
              uVar15 = (uint)((ulonglong)lVar8 >> 0x20);
              uVar13 = uVar13 - uVar6;
              uVar5 = CONCAT44(local_2ec,uStack_2e8);
              uVar6 = local_300;
              lVar7 = local_320;
              if (uStack_2dc < uVar15 || uVar15 - uStack_2dc < (uint)(local_2e0 <= (uint)lVar8)) {
                uVar13 = uVar3;
                uVar5 = CONCAT44(local_2ec,uStack_2e8);
                lVar8 = 0;
                if (lVar1 != 0) {
                  firehose_log("%llu sectors more to hash",(int)((ulonglong)uVar14 >> 0x20),
                               (int)lVar1,(int)((ulonglong)lVar1 >> 0x20));
                  uVar5 = CONCAT44(local_2ec,uStack_2e8);
                  uVar13 = uVar4;
                  uVar6 = local_300;
                  lVar7 = local_320;
                  lVar8 = 0;
                }
              }
            }
            if (iVar12 == 1) {
              local_320 = lVar7;
              if (local_28 == 0) {
                FUN_0803649c(auStack_3b0,puStack_304,local_308,auStack_4c);
                uVar5 = CONCAT44(local_2ec,uStack_2e8);
              }
              local_2ec = (undefined4)(uVar5 >> 0x20);
              uStack_2e8 = (undefined4)uVar5;
              uVar15 = snprintf_buf(acStack_d4,0x48,&DAT_0802387c,"Digest ");
              if (local_28 == 0) {
                for (; (uVar15 < 0x48 && (local_314 < 0x20)); local_314 = local_314 + 1) {
                  iVar12 = snprintf_buf(acStack_d4 + uVar15,0x48 - uVar15,&DAT_080238b0,
                                        auStack_4c[local_314]);
                  uVar15 = uVar15 + iVar12;
                }
              }
              else {
                snprintf_buf(acStack_d4 + uVar15,0x48 - uVar15,&DAT_08023880,local_30c);
              }
              firehose_log("Hash start sector %llu num sectors %llu",local_2ec,uStack_2f0,local_2f4,
                           uStack_2e8,local_2ec);
              pcVar10 = acStack_d4;
LAB_080236a2:
              firehose_log(pcVar10);
              uVar5 = CONCAT44(local_2ec,uStack_2e8);
              uVar6 = local_300;
              lVar7 = local_320;
            }
            local_2ec = (undefined4)(uVar5 >> 0x20);
            uStack_2e8 = (undefined4)uVar5;
            local_300 = uVar6;
            local_320 = lVar7;
            handle_response(1);
            uVar11 = 0;
            goto LAB_080236ae;
          }
          firehose_log("Failed to parse start sector");
        }
      }
      else {
        firehose_log("Sector size in XML %d does not match disk sector size %d",local_324,
                     DAT_08058458,DAT_0805845c);
      }
      goto LAB_08023522;
    }
  }
  else {
LAB_08023522:
    handle_response(0);
  }
  uVar11 = 1;
LAB_080236ae:
  if (local_2c != local_2f8) {
    FUN_08010960();
  }
  return uVar11;
LAB_080234c0:
  iVar9 = FUN_0803906c(&DAT_08055ea0,"SECTOR_SIZE_IN_BYTES");
  if (iVar9 != 0) {
    FUN_08038d38(&DAT_08055ea0,auStack_2d4,0x200,0);
    local_324 = FUN_0803823e(auStack_2d4,local_318);
    if (local_318[0] == '\0') {
      pcVar10 = "Failed to get sector_size_in_bytes";
LAB_080234e8:
      firehose_log(pcVar10);
      bVar2 = false;
    }
  }
  goto LAB_080234ee;
}




/* 0x080238b8 */
undefined4 handle_getstorageinfo()
{
  bool bVar1;
  int iVar2;
  int iVar3;
  uint uVar4;
  undefined4 uVar5;
  char local_220 [4];
  undefined1 auStack_21c [512];
  int local_1c;
  
  iVar2 = DAT_0804cd58;
  local_1c = DAT_0804cd58;
  FUN_08006d14(auStack_21c,0x200);
  bVar1 = true;
  uVar4 = 0xffffffff;
  while (iVar3 = FUN_08038db4(&DAT_08055ea0), iVar3 == 2) {
    iVar3 = FUN_0803906c(&DAT_08055ea0,"physical_partition_number");
    if (iVar3 != 0) {
      FUN_08038d38(&DAT_08055ea0,auStack_21c,0x200,0);
      uVar4 = FUN_0803823e(auStack_21c,local_220);
      if (local_220[0] == '\0') {
        firehose_log("Failed to get physical partition number");
        bVar1 = false;
      }
    }
  }
  if ((uVar4 != 0xffffffff) && (bVar1)) {
    iVar3 = FUN_08038206(&DAT_08058430,uVar4 & 0xff);
    if (iVar3 == 0) {
      firehose_log("Invalid physical partition %d",uVar4);
    }
    else {
      iVar3 = FUN_08037f88(&DAT_08058430);
      if (iVar3 != 0) {
        handle_response(1);
        uVar5 = 0;
        goto LAB_08023938;
      }
    }
  }
  handle_response(0);
  uVar5 = 1;
LAB_08023938:
  if (local_1c != iVar2) {
    FUN_08010960();
  }
  return uVar5;
}




/* 0x080239d0 */
undefined4 handle_nop()
{
  handle_response(1);
  return 0;
}




/* 0x080239dc */
undefined4 handle_patch()
{
  bool bVar1;
  char *pcVar2;
  uint uVar3;
  int iVar4;
  uint uVar5;
  uint uVar6;
  undefined1 *puVar7;
  undefined4 extraout_r1;
  int extraout_r2;
  int extraout_r3;
  undefined4 uVar8;
  uint uVar9;
  int iVar10;
  undefined8 uVar11;
  longlong lVar12;
  undefined *puVar13;
  char local_314 [4];
  undefined4 local_310;
  undefined4 local_30c;
  undefined4 local_308;
  undefined4 uStack_304;
  undefined4 local_300;
  undefined4 local_2fc;
  undefined *local_2f8;
  int local_2f4;
  undefined4 *local_2f0;
  undefined1 auStack_2ec [3];
  undefined1 auStack_2e9 [513];
  undefined1 uStack_e8;
  undefined1 auStack_e7 [63];
  undefined1 auStack_a8 [64];
  undefined1 auStack_68 [64];
  int local_28;
  
  local_2f4 = DAT_0804cd58;
  local_28 = DAT_0804cd58;
  uVar9 = 0xffffffff;
  iVar10 = 0x200;
  local_308 = 0;
  uStack_304 = 0;
  bVar1 = true;
  FUN_08006d14(auStack_68,0x40);
  FUN_08006d14(&uStack_e8,0x40);
  FUN_08006d14(auStack_a8,0x40);
  FUN_08006d14(auStack_2e9 + 1,0x200);
  local_310 = 0;
  local_30c = 0;
  uVar5 = uVar9;
  uVar3 = uVar9;
  while (iVar4 = FUN_08038db4(&DAT_08055ea0), iVar4 == 2) {
    iVar4 = FUN_0803906c(&DAT_08055ea0,"byte_offset");
    if (iVar4 == 0) {
      iVar4 = FUN_0803906c(&DAT_08055ea0,"physical_partition_number");
      if (iVar4 == 0) {
        iVar4 = FUN_0803906c(&DAT_08055ea0,"size_in_bytes");
        if (iVar4 == 0) {
          iVar4 = FUN_0803906c(&DAT_08055ea0,"start_sector");
          if (iVar4 != 0) {
            puVar7 = auStack_a8;
            goto LAB_08023b14;
          }
          iVar4 = FUN_0803906c(&DAT_08055ea0,"SECTOR_SIZE_IN_BYTES");
          if (iVar4 == 0) {
            iVar4 = FUN_0803906c(&DAT_08055ea0,"filename");
            if (iVar4 != 0) {
              puVar7 = auStack_68;
              goto LAB_08023b14;
            }
            iVar4 = FUN_0803906c(&DAT_08055ea0,"value");
            if (iVar4 != 0) {
              puVar7 = &uStack_e8;
LAB_08023b14:
              FUN_08038d38(&DAT_08055ea0,puVar7,0x40,0);
            }
          }
          else {
            FUN_08038d38(&DAT_08055ea0,auStack_2e9 + 1,0x200,0);
            iVar10 = FUN_0803823e(auStack_2e9 + 1,local_314);
            if (local_314[0] == '\0') {
              pcVar2 = "Failed to get sector_size_in_bytes";
              goto LAB_08023aea;
            }
          }
        }
        else {
          FUN_08038d38(&DAT_08055ea0,auStack_2e9 + 1,0x200,0);
          uVar3 = FUN_0803823e(auStack_2e9 + 1,local_314);
          if (local_314[0] == '\0') {
            pcVar2 = "Failed to get size_in_bytes";
            goto LAB_08023aea;
          }
        }
      }
      else {
        FUN_08038d38(&DAT_08055ea0,auStack_2e9 + 1,0x200,0);
        uVar5 = FUN_0803823e(auStack_2e9 + 1,local_314);
        if (local_314[0] == '\0') {
          pcVar2 = "Failed to get physical_partition_number";
          goto LAB_08023aea;
        }
      }
    }
    else {
      FUN_08038d38(&DAT_08055ea0,auStack_2e9 + 1,0x200,0);
      uVar9 = FUN_0803823e(auStack_2e9 + 1,local_314);
      if (local_314[0] == '\0') {
        pcVar2 = "Failed to get byte_offset";
LAB_08023aea:
        firehose_log(pcVar2);
        bVar1 = false;
      }
    }
  }
  if (!bVar1) goto LAB_08023b48;
  if ((((uVar9 == 0xffffffff) || (uVar5 == 0xffffffff)) || (uVar3 == 0xffffffff)) || (iVar10 == -1))
  goto LAB_08023c14;
  iVar4 = strncmp_fh(&DAT_08023e4c,auStack_68,0x40);
  if (iVar4 != 0) {
    uVar8 = 0;
    goto LAB_08023d36;
  }
  local_2f0 = &DAT_08058458;
  if (DAT_08058458 == iVar10 && DAT_0805845c == iVar10 >> 0x1f) {
    local_2f8 = &DAT_08058430;
    iVar10 = FUN_08038206(&DAT_08058430,uVar5 & 0xff);
    if (iVar10 == 0) {
      firehose_log("Invalid physical partition %d",uVar5);
      goto LAB_08023b48;
    }
    puVar13 = &DAT_08055e90;
    iVar4 = (&DAT_08054e90)[1 - DAT_08055fd4];
    FUN_08037f64(local_2f8);
    snprintf_buf(auStack_2e9 + 1,0x200,&DAT_08023e98);
    FUN_0803725c(&uStack_e8,"NUM_DISK_SECTORS",auStack_2e9 + 1);
    FUN_0803725c(&uStack_e8,&DAT_08023eb4,"");
    FUN_0803725c(auStack_a8,"NUM_DISK_SECTORS",auStack_2e9 + 1);
    FUN_0803725c(auStack_a8,&DAT_08023eb4,"");
    FUN_080288b0(auStack_a8,&local_308);
    iVar10 = FUN_08006874(&uStack_e8,"CRC32");
    if (iVar10 == 0) {
      FUN_080288b0(&uStack_e8,&local_310);
LAB_08023cd4:
      FUN_08038014(local_2f8,iVar4,local_308,uStack_304,1,0);
      if (uVar3 == 4) {
        local_30c = 0;
      }
      FUN_08027bf8(iVar4 + uVar9,*(int *)(puVar13 + 0x128) - uVar9,&local_310,uVar3);
      mmc_write_wrapper(local_2f8,iVar4,local_308,uStack_304,1,0,auStack_2ec);
      firehose_log("Patched sector %llu with %08X",local_30c,local_308,uStack_304,local_310,
                   local_30c);
      uVar8 = handle_response(1);
      goto LAB_08023d36;
    }
    FUN_0803725c(&uStack_e8,"CRC32","");
    iVar10 = FUN_08006906(&uStack_e8);
    uVar5 = FUN_08006860(&uStack_e8,0x2c);
    uVar6 = FUN_080068f0(&uStack_e8,0x29);
    if ((uVar5 == 0) || (uVar6 == 0)) {
      firehose_log("Invalid CRC patch value");
    }
    else if (uVar5 <= uVar6) {
      FUN_08027bf8(auStack_2e9 + 1,0x200,uVar5 + 1,(uVar6 - uVar5) + -1);
      auStack_2e9[uVar6 - uVar5] = 0;
      uVar11 = FUN_0803823e(auStack_2e9 + 1,local_314);
      if (local_314[0] == '\0') {
        firehose_log("Failed to get crc_over_num_bytes");
        goto LAB_08023b48;
      }
      FUN_08027c12(uVar5,0x200,uVar6,auStack_e7 + (iVar10 - uVar6));
      FUN_080288b0(&uStack_e8,&local_300);
      firehose_log("crc start sector %llu, over bytes %llu",extraout_r1,local_300,local_2fc,uVar11);
      lVar12 = FUN_080060c0((int)uVar11,(int)((ulonglong)uVar11 >> 0x20),*local_2f0,local_2f0[1]);
      FUN_08038014(local_2f8,iVar4,local_300,local_2fc,
                   lVar12 + (ulonglong)(extraout_r2 != 0 || extraout_r3 != 0));
      local_310 = FUN_08019abc(iVar4,(int)uVar11);
      local_30c = 0;
      goto LAB_08023cd4;
    }
  }
  else {
    firehose_log("Sector size in XML %d does not match disk sector size %d",iVar10);
LAB_08023b48:
    handle_response(0);
  }
LAB_08023c14:
  uVar8 = 1;
LAB_08023d36:
  if (local_28 != local_2f4) {
    FUN_08010960();
  }
  return uVar8;
}




/* 0x08023f64 */
undefined4 handle_peek()
{
  longlong lVar1;
  char *pcVar2;
  int iVar3;
  int extraout_r2;
  int extraout_r3;
  undefined4 uVar4;
  uint uVar5;
  uint uVar6;
  uint uVar7;
  int iVar8;
  bool bVar9;
  longlong lVar10;
  char local_438 [4];
  int local_434;
  undefined4 local_430;
  undefined1 auStack_42c [512];
  undefined1 local_22c [512];
  int local_2c;
  
  local_434 = DAT_0804cd58;
  local_2c = DAT_0804cd58;
  FUN_08006d14(auStack_42c,0x200);
  FUN_08006d14(local_22c,0x200);
  iVar8 = 0;
  bVar9 = true;
  lVar10 = 0;
  local_430 = 0x33;
LAB_08023ffc:
  while( true ) {
    uVar7 = (uint)((ulonglong)lVar10 >> 0x20);
    iVar3 = FUN_08038db4(&DAT_08055ea0);
    if (iVar3 != 2) break;
    iVar3 = FUN_0803906c(&DAT_08055ea0,"SizeInBytes");
    if (iVar3 == 0) goto LAB_08023fcc;
    FUN_08038d38(&DAT_08055ea0,auStack_42c,0x200,0);
    lVar10 = FUN_0803823e(auStack_42c,local_438);
    if (local_438[0] == '\0') {
      pcVar2 = "Failed to get size in bytes";
      goto LAB_08023ff6;
    }
  }
  if (bVar9) {
    if ((iVar8 != 0) && (lVar10 != 0)) {
      firehose_log("Using address %p",iVar8);
      uVar5 = 0;
      uVar6 = 0;
      while (lVar1 = CONCAT44(uVar6,uVar5),
            uVar6 <= uVar7 && (uint)((uint)lVar10 <= uVar5) <= uVar6 - uVar7) {
        snprintf_buf(auStack_42c,0x200,"0x%02X ",*(undefined1 *)(iVar8 + uVar5));
        FUN_08038282(local_22c,auStack_42c,0x200);
        bVar9 = 0xfffffffe < uVar5;
        uVar5 = uVar5 + 1;
        uVar6 = uVar6 + bVar9;
        FUN_080060c0(uVar5,uVar6,local_430,0);
        if ((extraout_r2 == 0 && extraout_r3 == 0) || (lVar10 + -1 == lVar1)) {
          firehose_log(&DAT_08024130,local_22c);
          local_22c[0] = 0;
        }
      }
      handle_response(1);
      uVar4 = 0;
      goto LAB_08024094;
    }
    firehose_log("Invalid parameters");
  }
  handle_response(0);
  uVar4 = 1;
LAB_08024094:
  if (local_2c != local_434) {
    FUN_08010960();
  }
  return uVar4;
LAB_08023fcc:
  iVar3 = FUN_0803906c(&DAT_08055ea0,"address64");
  if (iVar3 != 0) {
    FUN_08038d38(&DAT_08055ea0,auStack_42c,0x200,0);
    iVar8 = FUN_0803823e(auStack_42c,local_438);
    if (local_438[0] == '\0') {
      pcVar2 = "Failed to get address";
LAB_08023ff6:
      firehose_log(pcVar2);
      bVar9 = false;
    }
  }
  goto LAB_08023ffc;
}




/* 0x08024134 */
undefined4 handle_poke()
{
  int iVar1;
  char *pcVar2;
  int iVar3;
  uint uVar4;
  uint uVar5;
  undefined1 *puVar6;
  undefined4 uVar7;
  uint uVar8;
  uint uVar9;
  bool bVar10;
  longlong lVar11;
  ulonglong uVar12;
  char local_230 [4];
  undefined1 auStack_22c [512];
  int local_2c;
  
  iVar1 = DAT_0804cd58;
  local_2c = DAT_0804cd58;
  FUN_08006d14(auStack_22c,0x200);
  puVar6 = (undefined1 *)0x0;
  bVar10 = true;
  lVar11 = 0;
  uVar12 = 0;
LAB_080241c2:
  while (uVar9 = (uint)((ulonglong)lVar11 >> 0x20), uVar8 = (uint)lVar11,
        iVar3 = FUN_08038db4(&DAT_08055ea0), iVar3 == 2) {
    iVar3 = FUN_0803906c(&DAT_08055ea0,"SizeInBytes");
    if (iVar3 != 0) {
      FUN_08038d38(&DAT_08055ea0,auStack_22c,0x200,0);
      lVar11 = FUN_0803823e(auStack_22c,local_230);
      goto LAB_0802420e;
    }
    iVar3 = FUN_0803906c(&DAT_08055ea0,"address64");
    if (iVar3 == 0) goto LAB_080241e8;
    FUN_08038d38(&DAT_08055ea0,auStack_22c,0x200,0);
    puVar6 = (undefined1 *)FUN_0803823e(auStack_22c,local_230);
    if (local_230[0] == '\0') {
      pcVar2 = "Failed to get address";
      goto LAB_080241ba;
    }
  }
  if (bVar10) {
    if ((puVar6 == (undefined1 *)0x0) || (lVar11 == 0)) {
      firehose_log("Invalid parameters address %p size_in_bytes %llu",puVar6,uVar8,uVar9);
    }
    else {
      if (-uVar9 < (uint)(uVar8 < 9)) {
        firehose_log("Using address %p",puVar6);
        uVar5 = 0;
        for (uVar4 = 0; uVar4 <= uVar9 && (uint)(uVar8 <= uVar5) <= uVar4 - uVar9;
            uVar4 = uVar4 + bVar10) {
          *puVar6 = (char)uVar12;
          bVar10 = 0xfffffffe < uVar5;
          uVar5 = uVar5 + 1;
          uVar12 = uVar12 >> 8;
          puVar6 = puVar6 + 1;
        }
        handle_response(1,uVar5,(uVar4 - uVar9) - (uint)(uVar8 > uVar5));
        uVar7 = 0;
        goto LAB_08024266;
      }
      firehose_log("Cannot handle size in bytes greater than %llu",8);
    }
  }
  handle_response(0);
  uVar7 = 1;
LAB_08024266:
  if (local_2c != iVar1) {
    FUN_08010960();
  }
  return uVar7;
LAB_080241e8:
  iVar3 = FUN_0803906c(&DAT_08055ea0,"value");
  if (iVar3 != 0) {
    FUN_08038d38(&DAT_08055ea0,auStack_22c,0x200,0);
    uVar12 = FUN_0803823e(auStack_22c,local_230);
LAB_0802420e:
    if (local_230[0] == '\0') {
      pcVar2 = "Failed to get size in bytes";
LAB_080241ba:
      firehose_log(pcVar2);
      bVar10 = false;
    }
  }
  goto LAB_080241c2;
}




/* 0x08024350 */
undefined4 handle_power()
{
  int iVar1;
  int iVar2;
  int iVar3;
  int iVar4;
  char local_220 [4];
  undefined1 auStack_21c [512];
  int local_1c;
  
  iVar1 = DAT_0804cd58;
  local_1c = DAT_0804cd58;
  FUN_08006d14(auStack_21c,0x200);
  iVar3 = 0;
  iVar4 = 0;
LAB_080243d6:
  do {
    while( true ) {
      iVar2 = FUN_08038db4(&DAT_08055ea0);
      if (iVar2 != 2) {
        if (iVar4 == 1) {
          handle_response(1);
          FUN_0801b85c(iVar3);
          FUN_0801999c();
          do {
                    /* WARNING: Do nothing block with infinite loop */
          } while( true );
        }
        if (iVar4 != 2) {
          if (local_1c != iVar1) {
            FUN_08010960();
          }
          return 1;
        }
        handle_response(1);
        FUN_0801b85c(iVar3);
        FUN_08019984();
        do {
                    /* WARNING: Do nothing block with infinite loop */
        } while( true );
      }
      iVar2 = FUN_0803906c(&DAT_08055ea0,"value");
      if (iVar2 == 0) break;
      FUN_08038d38(&DAT_08055ea0,auStack_21c,0x200,0);
      iVar2 = strncmp_fh(auStack_21c,"reset",5);
      if (iVar2 == 0) {
        iVar4 = 1;
      }
      else {
        iVar2 = strncmp_fh(auStack_21c,&DAT_08024444,3);
        if (iVar2 == 0) {
          iVar4 = 2;
        }
      }
    }
    iVar2 = FUN_0803906c(&DAT_08055ea0,"delayinseconds");
  } while (iVar2 == 0);
  FUN_08038d38(&DAT_08055ea0,auStack_21c,0x200,0);
  iVar3 = FUN_0803823e(auStack_21c,local_220);
  if (local_220[0] != '\0') goto code_r0x080243d0;
  firehose_log("Failed to get delay time. Using default.");
  goto LAB_080243d4;
code_r0x080243d0:
  if (iVar3 < 1) {
LAB_080243d4:
    iVar3 = 1;
  }
  goto LAB_080243d6;
}




/* 0x08024484 */
int handle_program()
{
  undefined1 *puVar1;
  longlong lVar2;
  longlong lVar3;
  int iVar4;
  int iVar5;
  char *pcVar6;
  uint uVar7;
  uint uVar8;
  undefined4 uVar9;
  uint uVar10;
  int extraout_r2;
  uint uVar11;
  int extraout_r3;
  int iVar12;
  uint uVar13;
  int iVar14;
  int iVar15;
  bool bVar16;
  longlong lVar17;
  undefined8 uVar18;
  longlong lVar19;
  int local_32c;
  undefined4 local_328;
  undefined4 uStack_324;
  undefined4 local_320;
  undefined *local_31c;
  char local_318 [4];
  int local_314;
  uint local_310;
  uint uStack_30c;
  uint local_308;
  uint uStack_304;
  int local_300;
  int local_2fc;
  undefined *local_2f4;
  int *local_2f0;
  char local_2ec [512];
  undefined1 auStack_ec [64];
  undefined1 auStack_ac [64];
  undefined1 auStack_6c [32];
  undefined1 auStack_4c [32];
  int local_2c;
  
  local_2fc = DAT_0804cd58;
  local_2c = DAT_0804cd58;
  FUN_08006d14(local_2ec,0x200);
  iVar14 = 0x200;
  uVar13 = 0xffffffff;
  lVar2 = 0;
  local_310 = 0;
  uStack_30c = 0;
  FUN_08006d14(auStack_ec,0x40);
  FUN_08006d14(auStack_ac,0x40);
  local_314 = 0;
  local_300 = 1;
  FUN_08006d14(auStack_6c,0x20);
  FUN_08006d14(auStack_4c,0x20);
  bVar16 = true;
  iVar15 = 0;
  while (iVar4 = FUN_08038db4(&DAT_08055ea0), iVar4 == 2) {
    iVar4 = FUN_0803906c(&DAT_08055ea0,"start_sector");
    if (iVar4 == 0) {
      iVar4 = FUN_0803906c(&DAT_08055ea0,"num_partition_sectors");
      if (iVar4 != 0) {
        puVar1 = &stack0x00000268;
        goto LAB_08024502;
      }
      iVar4 = FUN_0803906c(&DAT_08055ea0,"physical_partition_number");
      if (iVar4 == 0) {
        iVar4 = FUN_0803906c(&DAT_08055ea0,"SECTOR_SIZE_IN_BYTES");
        if (iVar4 == 0) {
          iVar4 = FUN_0803906c(&DAT_08055ea0,"read_back_verify");
          if ((iVar4 != 0) && (FUN_08038d38(&DAT_08055ea0,local_2ec,0x200,0), local_2ec[0] == '1'))
          {
            local_314 = 1;
          }
        }
        else {
          FUN_08038d38(&DAT_08055ea0,local_2ec,0x200,0);
          iVar14 = FUN_0803823e(local_2ec,local_318);
          if (local_318[0] == '\0') {
            pcVar6 = "Failed to get sector size";
            goto LAB_0802455e;
          }
        }
      }
      else {
        FUN_08038d38(&DAT_08055ea0,local_2ec,0x200,0);
        uVar13 = FUN_0803823e(local_2ec,local_318);
        if (local_318[0] == '\0') {
          pcVar6 = "Failed to get physical partition number";
LAB_0802455e:
          firehose_log(pcVar6);
          bVar16 = false;
        }
      }
    }
    else {
      puVar1 = &stack0x00000228;
LAB_08024502:
      FUN_08038d38(&DAT_08055ea0,puVar1 + -0x314,0x40,0);
    }
  }
  if (bVar16) {
    if (uVar13 != 0xffffffff) {
      iVar4 = iVar14 >> 0x1f;
      if (DAT_08058458 == iVar14 && DAT_0805845c == iVar4) {
        local_2f4 = &DAT_08055e90;
        lVar17 = FUN_080060c0(DAT_08055fb8,DAT_08055fbc,iVar14,iVar4);
        uVar7 = (uint)((ulonglong)lVar17 >> 0x20);
        local_31c = &DAT_08058430;
        uVar18 = FUN_08038206(&DAT_08058430,uVar13 & 0xff);
        if ((int)uVar18 == 0) {
          firehose_log("Invalid physical partition %d",uVar13);
        }
        else {
          uVar18 = FUN_08028c10(auStack_ec,(int)((ulonglong)uVar18 >> 0x20),0x40,0);
          if ((int)uVar18 == 0) {
            pcVar6 = "Failed to parse start sector";
          }
          else {
            iVar12 = FUN_08028c10(auStack_ac,(int)((ulonglong)uVar18 >> 0x20),0x40,0);
            if (iVar12 != 0) {
              firehose_log("start %llu, num %llu",uStack_30c,0,0);
              FUN_08037084(1,1,"rawmode",0x73);
              local_32c = 1;
              local_2f0 = &DAT_08058484;
              while( true ) {
                uStack_324 = (undefined4)((ulonglong)lVar2 >> 0x20);
                local_328 = (undefined4)lVar2;
                if (local_310 == 0 && uStack_30c == 0) break;
                lVar19 = CONCAT44(uStack_30c,local_310);
                if (uVar7 < uStack_30c || uStack_30c - uVar7 < (uint)((uint)lVar17 <= local_310)) {
                  lVar19 = lVar17;
                }
                uVar10 = (uint)(lVar19 * iVar14);
                uVar11 = (uint)((ulonglong)(lVar19 * iVar14) >> 0x20);
                iVar12 = FUN_08021ca4(&local_320);
                FUN_0801b6bc("toread %d read %d",uStack_304,uVar10,uVar11);
                uVar8 = uStack_304;
                uVar13 = local_308;
                if (iVar12 != 0) {
                  if (local_2f4[0x148] == '\x01') {
                    firehose_log("Encountered RAW data transport error in validation mode.");
                  }
                  else {
                    FUN_0801b85c(10);
                    FUN_08037084(0,2,"rawmode",0x73);
                  }
                  goto LAB_08024840;
                }
                if ((uStack_304 < uVar11 || uVar11 - uStack_304 < (uint)(local_308 <= uVar10)) &&
                   (FUN_080060c0(local_308,uStack_304,iVar14,iVar4),
                   extraout_r2 == 0 && extraout_r3 == 0)) {
                  lVar19 = FUN_080060c0(uVar13,uVar8,iVar14,iVar4);
                  iVar12 = (int)((ulonglong)lVar19 >> 0x20);
                  uVar13 = (uint)lVar19;
                  FUN_0801b6bc("rx %llu start %llu",uStack_324,uVar13,iVar12);
                  if (lVar19 != CONCAT44(uStack_30c,local_310)) {
                    uVar8 = (uStack_30c - iVar12) - (uint)(local_310 < uVar13);
                    lVar3 = CONCAT44(uVar8,local_310 - uVar13);
                    if (uVar7 < uVar8 || uVar8 - uVar7 < (uint)((uint)lVar17 <= local_310 - uVar13))
                    {
                      lVar3 = lVar17;
                    }
                    FUN_08030ecc((int)(lVar3 * iVar14),(int)((ulonglong)(lVar3 * iVar14) >> 0x20));
                  }
                  if (local_32c == 1) {
                    if (DAT_08058480 == '\x01') goto LAB_080247c4;
                    uVar18 = mmc_write_wrapper(local_31c,local_320,local_328,uStack_324);
                    uVar9 = (undefined4)((ulonglong)uVar18 >> 0x20);
                    iVar5 = (int)uVar18;
                    if (iVar5 == 0) {
                      pcVar6 = "Write failed Sector %llu, Errno %d";
LAB_080247b6:
                      firehose_log(pcVar6,uVar9,local_328,uStack_324);
LAB_080247be:
                      local_32c = 0;
                    }
                    else {
                      if (iVar5 == 1) {
                        if (local_300 == 1) {
                          local_300 = 0;
                        }
                        else if (local_314 != 1) goto LAB_080247c4;
                        FUN_0800d59c(local_320,local_308,auStack_6c);
                        FUN_08038014(local_31c,local_320,local_328,uStack_324);
                        FUN_0800d59c(local_320,local_308,auStack_4c);
                        uVar18 = FUN_08006898(auStack_6c,auStack_4c,0x20);
                        uVar9 = (undefined4)((ulonglong)uVar18 >> 0x20);
                        if ((int)uVar18 != 0) {
                          pcVar6 = "Read back verify failed at sector %d,num sectors %d";
                          goto LAB_080247b6;
                        }
                      }
                      else if (iVar5 == 0) goto LAB_080247be;
LAB_080247c4:
                      lVar2 = lVar19 + lVar2;
                    }
                  }
                  bVar16 = local_310 < uVar13;
                  local_310 = local_310 - uVar13;
                  uStack_30c = (uStack_30c - iVar12) - (uint)bVar16;
                  if (DAT_08058482 == '\x01') {
                    firehose_log("%d bytes received, %d sectors more to go",local_310,local_308,
                                 uStack_304);
                  }
                  if ((*local_2f0 != 0) && (iVar15 = iVar15 + 1, *local_2f0 == iVar15)) {
                    handle_response(local_32c);
                    iVar15 = 0;
                  }
                }
              }
              firehose_log("Finished sector address %llu",0,local_328,uStack_324);
              FUN_08037084(local_32c,1,"rawmode",0x73);
              if (local_32c == 1) {
                iVar12 = 0;
                goto LAB_08024840;
              }
              goto LAB_0802483a;
            }
            pcVar6 = "Failed to parse num_partition_sectors";
          }
          firehose_log(pcVar6);
        }
      }
      else {
        firehose_log("Sector size in XML %d does not match disk sector size %d",iVar14);
      }
      goto LAB_080245b2;
    }
  }
  else {
LAB_080245b2:
    handle_response(0);
  }
LAB_0802483a:
  iVar12 = 1;
LAB_08024840:
  if (local_2c != local_2fc) {
    FUN_08010960();
  }
  return iVar12;
}




/* 0x08024b04 */
undefined4 handle_read()
{
  bool bVar1;
  longlong lVar2;
  longlong lVar3;
  char *pcVar4;
  int iVar5;
  int iVar6;
  undefined4 extraout_r1;
  undefined4 uVar7;
  int iVar8;
  uint uVar9;
  longlong lVar10;
  uint local_298;
  undefined4 uStack_294;
  undefined4 local_290;
  undefined *local_28c;
  undefined8 local_288;
  uint local_280;
  undefined4 uStack_27c;
  char local_278 [4];
  int local_274;
  undefined *local_270;
  undefined1 auStack_26c [512];
  undefined1 auStack_6c [64];
  int local_2c;
  
  local_274 = DAT_0804cd58;
  local_2c = DAT_0804cd58;
  iVar8 = 0x200;
  FUN_08006d14(auStack_26c,0x200);
  uVar9 = 0xffffffff;
  bVar1 = true;
  lVar10 = 0;
  local_298 = 0;
  uStack_294 = 0;
  local_280 = 0;
  uStack_27c = 0;
  FUN_08006d14(auStack_6c,0x40);
LAB_08024be4:
  while( true ) {
    iVar5 = FUN_08038db4(&DAT_08055ea0);
    if (iVar5 != 2) break;
    iVar5 = FUN_0803906c(&DAT_08055ea0,"start_sector");
    if (iVar5 == 0) {
      iVar5 = FUN_0803906c(&DAT_08055ea0,"num_partition_sectors");
      if (iVar5 == 0) {
        iVar5 = FUN_0803906c(&DAT_08055ea0,"physical_partition_number");
        if (iVar5 == 0) goto LAB_08024bb6;
        FUN_08038d38(&DAT_08055ea0,auStack_26c,0x200,0);
        uVar9 = FUN_0803823e(auStack_26c,local_278);
        if (local_278[0] == '\0') {
          pcVar4 = "Failed to get physical_partition_number";
          goto LAB_08024bde;
        }
      }
      else {
        FUN_08038d38(&DAT_08055ea0,auStack_26c,0x200,0);
        lVar10 = FUN_0803823e(auStack_26c,local_278);
        if (local_278[0] == '\0') {
          pcVar4 = "Failed to get num_partition_sectors";
          goto LAB_08024bde;
        }
      }
    }
    else {
      FUN_08038d38(&DAT_08055ea0,auStack_6c,0x40,0);
    }
  }
  if (bVar1) {
    if (uVar9 != 0xffffffff) {
      if (DAT_08058458 == iVar8 && DAT_0805845c == iVar8 >> 0x1f) {
        local_270 = &DAT_08055e90;
        local_288 = FUN_080060c0(DAT_08055fb8,DAT_08055fbc,iVar8,iVar8 >> 0x1f);
        local_28c = &DAT_08058430;
        iVar5 = FUN_08038206(&DAT_08058430,uVar9 & 0xff);
        if (iVar5 == 0) {
          firehose_log("Invalid physical partition %d",uVar9);
        }
        else {
          iVar5 = FUN_08028c10(auStack_6c,&local_298,0x40,0,&local_298);
          if (iVar5 != 0) {
            FUN_08037084(1,1,"rawmode",0x73,&DAT_08024e68);
            iVar5 = 1;
            lVar2 = local_288;
            while( true ) {
              _GHIDRA_FIELD(local_288, 4, uint) = (uint)((ulonglong)lVar2 >> 0x20);
              _GHIDRA_FIELD(local_288, 0, uint) = (uint)lVar2;
              uVar9 = (uint)((ulonglong)lVar10 >> 0x20);
              local_288 = lVar2;
              if (lVar10 == 0) break;
              if (uVar9 <= _GHIDRA_FIELD(local_288, 4, uint) &&
                  (uint)((uint)local_288 <= (uint)lVar10) <= uVar9 - _GHIDRA_FIELD(local_288, 4, uint)) {
                lVar2 = lVar10;
              }
              lVar3 = CONCAT44(uStack_294,&DAT_08054e90);
              local_290 = (&DAT_08054e90)[1 - *(int *)(local_270 + 0x144)];
              if (iVar5 == 1) {
                iVar6 = FUN_08038014(local_28c,local_290,local_298,uStack_294,lVar2);
                lVar3 = CONCAT44(uStack_294,local_298);
                if (iVar6 == 0) {
                  local_280 = local_298;
                  uStack_27c = uStack_294;
                  iVar5 = 0;
                }
                else {
                  lVar3 = lVar2 + lVar3;
                  local_298 = (uint)lVar3;
                }
              }
              uStack_294 = (undefined4)((ulonglong)lVar3 >> 0x20);
              FUN_08038c24(local_290,(int)lVar3,(int)(lVar2 * iVar8),
                           (int)((ulonglong)(lVar2 * iVar8) >> 0x20));
              lVar10 = lVar10 - lVar2;
              lVar2 = local_288;
            }
            if (iVar5 == 0) {
              firehose_log("Read failed at sector %llu",0,local_280,uStack_27c);
              uVar7 = extraout_r1;
            }
            else {
              uVar7 = 0;
            }
            firehose_log("Finished sector address %d",uVar7,local_298,uStack_294);
            FUN_08037084(iVar5,1,"rawmode",0x73,"false");
            if (iVar5 == 1) {
              uVar7 = 0;
              goto LAB_08024d3c;
            }
            goto LAB_08024d36;
          }
          firehose_log("Failed to parse start sector");
        }
      }
      else {
        firehose_log("Sector size in XML %d does not match disk sector size %d",iVar8,DAT_08058458);
      }
      goto LAB_08024c12;
    }
  }
  else {
LAB_08024c12:
    handle_response(0);
  }
LAB_08024d36:
  uVar7 = 1;
LAB_08024d3c:
  if (local_2c != local_274) {
    FUN_08010960();
  }
  return uVar7;
LAB_08024bb6:
  iVar5 = FUN_0803906c(&DAT_08055ea0,"SECTOR_SIZE_IN_BYTES");
  if (iVar5 != 0) {
    FUN_08038d38(&DAT_08055ea0,auStack_26c,0x200,0);
    iVar8 = FUN_0803823e(auStack_26c,local_278);
    if (local_278[0] == '\0') {
      pcVar4 = "Failed to get sector_size_in_bytes";
LAB_08024bde:
      firehose_log(pcVar4);
      bVar1 = false;
    }
  }
  goto LAB_08024be4;
}




/* 0x08024efc */
undefined4 handle_setbootable()
{
  bool bVar1;
  int iVar2;
  int iVar3;
  undefined4 uVar4;
  uint uVar5;
  char local_220 [4];
  undefined1 auStack_21c [512];
  int local_1c;
  
  iVar2 = DAT_0804cd58;
  local_1c = DAT_0804cd58;
  FUN_08006d14(auStack_21c,0x200);
  bVar1 = true;
  uVar5 = 0xffffffff;
  while (iVar3 = FUN_08038db4(&DAT_08055ea0), iVar3 == 2) {
    iVar3 = FUN_0803906c(&DAT_08055ea0,"value");
    if (iVar3 != 0) {
      FUN_08038d38(&DAT_08055ea0,auStack_21c,0x200,0);
      uVar5 = FUN_0803823e(auStack_21c,local_220);
      if (local_220[0] == '\0') {
        firehose_log("Failed to get bootable drive");
        bVar1 = false;
      }
    }
  }
  if (!bVar1) {
    handle_response(0);
    uVar4 = 1;
    goto LAB_08024f86;
  }
  if (uVar5 == 0xffffffff) {
    firehose_log("Could not find bootable drive value.");
LAB_08024fb4:
    uVar4 = 0;
  }
  else {
    iVar3 = FUN_08038206(&DAT_08058430,uVar5 & 0xff);
    if (iVar3 == 0) {
      firehose_log("Failed to set bootable drive to %d.",uVar5);
      goto LAB_08024fb4;
    }
    FUN_0803803c(&DAT_08058430);
    FUN_08037c40(&DAT_08058430);
    firehose_log("Set bootable drive to %d.",uVar5);
    uVar4 = 1;
  }
  uVar4 = handle_response(uVar4);
LAB_08024f86:
  if (local_1c != iVar2) {
    FUN_08010960();
  }
  return uVar4;
}




/* 0x08025054 */
undefined4 handle_storagextras()
{
  bool bVar1;
  bool bVar2;
  bool bVar3;
  int iVar4;
  int iVar5;
  undefined4 uVar6;
  char *pcVar7;
  undefined1 auStack_428 [4];
  char local_424 [512];
  undefined1 auStack_224 [512];
  int local_24;
  
  iVar4 = DAT_0804cd58;
  local_24 = DAT_0804cd58;
  FUN_08006d14(auStack_224,0x200);
  FUN_08006d14(local_424,0x200);
  bVar2 = false;
  bVar1 = true;
  bVar3 = false;
  while (iVar5 = FUN_08038db4(&DAT_08055ea0), iVar5 == 2) {
    iVar5 = FUN_0803906c(&DAT_08055ea0,"commit");
    if (iVar5 == 0) {
      iVar5 = FUN_0803906c(&DAT_08055ea0,"display");
      if (iVar5 == 0) {
        FUN_08038cfa(&DAT_08055ea0,auStack_224,0x200,0);
        FUN_08038d38(&DAT_08055ea0,local_424,0x200,0);
        iVar5 = FUN_0803805c(&DAT_08058430,auStack_224,local_424);
        if (iVar5 == 0) {
          bVar1 = false;
          firehose_log("Invalid storage extra parameter %s",auStack_224);
        }
      }
      else {
        FUN_08038d38(&DAT_08055ea0,local_424);
        if (local_424[0] == '1') {
          bVar3 = true;
        }
      }
    }
    else {
      FUN_08038d38(&DAT_08055ea0,local_424,0x200,0);
      if (local_424[0] == '1') {
        bVar2 = true;
      }
    }
  }
  if (bVar1) {
    if (bVar3) {
      FUN_08037e88(&DAT_08058430);
    }
    if (!bVar2) {
LAB_08025142:
      uVar6 = 1;
      goto LAB_08025144;
    }
    iVar5 = FUN_08038206(&DAT_08058430,0);
    if (iVar5 == 0) {
      pcVar7 = "Could not open partition for creating new drives";
    }
    else {
      iVar5 = FUN_08037c88(&DAT_08058430,auStack_428);
      if (iVar5 != 0) {
        FUN_08037c40(&DAT_08058430);
        firehose_log("Set storage parameters successfully");
        goto LAB_08025142;
      }
      pcVar7 = "Failed to set storage extra parameters";
    }
    firehose_log(pcVar7);
  }
  uVar6 = 0;
LAB_08025144:
  uVar6 = handle_response(uVar6);
  if (local_24 != iVar4) {
    FUN_08010960();
  }
  return uVar6;
}




/* 0x08025254 */
uint FUN_08025254(param_1, param_2)
undefined4 * param_1; int param_2;
{
  char cVar1;
  undefined4 *puVar2;
  byte *pbVar3;
  ushort uVar4;
  uint uVar5;
  uint uVar6;
  
  pbVar3 = (byte *)0x0;
  cVar1 = *(char *)(param_2 + 3);
  uVar6 = 0xffffffff;
  if (cVar1 == '\x01') {
    pbVar3 = (byte *)*param_1;
  }
  else {
    if (cVar1 == '\x02') {
      puVar2 = (undefined4 *)param_1[1];
      for (uVar4 = (ushort)*(byte *)(param_2 + 2); (puVar2 != (undefined4 *)0x0 && (uVar4 != 0));
          uVar4 = uVar4 - 1) {
        puVar2 = (undefined4 *)*puVar2;
      }
      if (puVar2 != (undefined4 *)0x0) {
        pbVar3 = (byte *)puVar2[1];
        uVar6 = (uint)*(ushort *)(pbVar3 + 2);
      }
      goto LAB_080252ba;
    }
    if (cVar1 != '\x03') {
      return 0xffffffff;
    }
    for (puVar2 = (undefined4 *)param_1[2];
        (puVar2 != (undefined4 *)0x0 &&
        ((*(short *)(puVar2 + 2) != *(short *)(param_2 + 4) ||
         (*(byte *)((int)puVar2 + 10) != *(byte *)(param_2 + 2))))); puVar2 = (undefined4 *)*puVar2)
    {
    }
    if (puVar2 == (undefined4 *)0x0) goto LAB_080252ba;
    pbVar3 = (byte *)puVar2[1];
  }
  uVar6 = (uint)*pbVar3;
LAB_080252ba:
  if (pbVar3 == (byte *)0x0) {
    return uVar6;
  }
  if ((int)(uint)*(ushort *)(param_2 + 6) < (int)uVar6) {
    uVar6 = (uint)*(ushort *)(param_2 + 6);
  }
  FUN_08027bf8(0x87c7473e,0x40,pbVar3,uVar6);
  uVar5 = 0xffffffff;
  if (false) {
    param_1 = param_1 + 0x1b;
  }
  else {
    param_1 = param_1 + 0xd;
  }
  if (param_1[1] != 1) {
    param_1[5] = 0x87c7473e;
    param_1[6] = uVar6;
    uVar5 = FUN_08030b04();
    if (-1 < (int)uVar5) {
      return uVar6;
    }
  }
  return uVar5;
}




/* 0x080252f0 */
int FUN_080252f0(param_1, param_2)
int param_1; int param_2;
{
  int iVar1;
  int iVar2;
  uint uVar3;
  int iVar4;
  
  if (false) {
switchD_080252fa_caseD_2:
    if (*(code **)(param_1 + 0x14) == (code *)0x0) {
      return -1;
    }
                    /* WARNING: Could not recover jumptable at 0x080253b2. Too many branches */
                    /* WARNING: Treating indirect jump as call */
    iVar1 = (**(code **)(param_1 + 0x14))();
    return iVar1;
  }
  switch(*(undefined1 *)(param_2 + 1)) {
  case 0:
    if (1 < *(ushort *)(param_2 + 6)) {
      **(undefined2 **)(param_1 + 0x30) = 0;
      iVar1 = 2;
LAB_08025390:
      iVar4 = -1;
      if (false) {
        iVar2 = param_1 + 0x6c;
      }
      else {
        iVar2 = param_1 + 0x34;
      }
      if (*(int *)(iVar2 + 4) != 1) {
        *(undefined4 *)(iVar2 + 0x14) = *(undefined4 *)(param_1 + 0x30);
        *(int *)(iVar2 + 0x18) = iVar1;
        iVar4 = FUN_08030b04();
        if (-1 < iVar4) {
          return iVar1;
        }
      }
      return iVar4;
    }
    break;
  case 1:
    if (*(short *)(param_2 + 2) != 2) {
      return -1;
    }
    goto LAB_08025328;
  default:
    goto switchD_080252fa_caseD_2;
  case 3:
    if (*(short *)(param_2 + 2) != 2) {
      return -1;
    }
    *(undefined1 *)(param_1 + 0xa4) = 1;
    uVar3 = ((uint)*(ushort *)(param_2 + 4) << 0x14) >> 0x1c;
    *(uint *)(param_1 + 0xa8) = uVar3;
    if (uVar3 < 6) {
      return 0;
    }
LAB_08025328:
    *(undefined4 *)(param_1 + 0xa8) = 0;
    return 0;
  case 5:
    if (*(char *)(param_1 + 0x29) == '\0') {
      *(undefined1 *)(param_1 + 0x28) = *(undefined1 *)(param_2 + 2);
      FUN_08030320(*(undefined4 *)(param_1 + 0xc));
      return 0;
    }
    return -1;
  case 6:
    iVar1 = FUN_08025254();
    return iVar1;
  case 7:
    break;
  case 8:
    if (*(short *)(param_2 + 6) != 0) {
      **(undefined1 **)(param_1 + 0x30) = *(undefined1 *)(param_1 + 0x29);
      iVar1 = 1;
      goto LAB_08025390;
    }
    break;
  case 9:
    iVar1 = FUN_0802fcd0(param_1,*(undefined1 *)(param_2 + 2));
    return iVar1;
  }
  return -1;
}




/* 0x080253ba */
int FUN_080253ba(param_1, param_2)
int param_1; int param_2;
{
  byte bVar1;
  char cVar2;
  int iVar3;
  undefined4 *puVar4;
  int iVar5;
  
  if ((*(int *)(param_1 + 0x2c) != 0) &&
     (iVar3 = *(int *)(*(int *)(param_1 + 0x2c) + 0xc), iVar3 != 0)) {
    for (puVar4 = *(undefined4 **)(iVar3 + 0xc); puVar4 != (undefined4 *)0x0;
        puVar4 = (undefined4 *)*puVar4) {
      if ((puVar4[1] != 0) && ((ushort)*(byte *)(puVar4[1] + 2) == *(ushort *)(param_2 + 4)))
      goto LAB_080253e4;
    }
  }
  puVar4 = (undefined4 *)0x0;
LAB_080253e4:
  if (puVar4 != (undefined4 *)0x0) {
    cVar2 = *(char *)(param_2 + 1);
    if (cVar2 == '\0') {
      if (1 < *(ushort *)(param_2 + 6)) {
        **(ushort **)(param_1 + 0x30) = (ushort)((uint)(puVar4[4] << 0x1f) >> 0x1f);
        iVar3 = -1;
        if (false) {
          iVar5 = param_1 + 0x6c;
        }
        else {
          iVar5 = param_1 + 0x34;
        }
        if (*(int *)(iVar5 + 4) != 1) {
          *(undefined4 *)(iVar5 + 0x14) = *(undefined4 *)(param_1 + 0x30);
          *(undefined4 *)(iVar5 + 0x18) = 2;
          iVar3 = FUN_08030b04();
          if (-1 < iVar3) {
            return 2;
          }
        }
        return iVar3;
      }
    }
    else if (cVar2 == '\x01') {
      if (*(short *)(param_2 + 2) == 0) {
        iVar3 = 0;
LAB_08025442:
        bVar1 = *(byte *)(puVar4[1] + 2);
        puVar4[4] = iVar3;
        if (iVar3 == 0) {
          FUN_08030614(*(undefined4 *)(param_1 + 0xc),bVar1 & 0xf,bVar1 >> 7);
        }
        else {
          FUN_080303c4();
        }
        return 0;
      }
    }
    else if (cVar2 == '\x03') {
      if (*(short *)(param_2 + 2) == 0) {
        iVar3 = 1;
        goto LAB_08025442;
      }
    }
    else if ((cVar2 != '\f') && ((code *)puVar4[3] != (code *)0x0)) {
                    /* WARNING: Could not recover jumptable at 0x08025400. Too many branches */
                    /* WARNING: Treating indirect jump as call */
      iVar3 = ((code *)puVar4[3])();
      return iVar3;
    }
  }
  return -1;
}




/* 0x0802544c */
int FUN_0802544c(param_1, param_2)
int param_1; int param_2;
{
  byte bVar1;
  int iVar2;
  int iVar3;
  int iVar4;
  
  iVar4 = *(int *)(param_1 + 0x2c);
  bVar1 = *(byte *)(param_2 + 1);
  iVar2 = *(int *)(iVar4 + 0xc);
  if (bVar1 == 3) {
    return -1;
  }
  if (bVar1 < 4) {
    if (bVar1 == 0) {
      if (iVar2 == 0) {
        return -1;
      }
      if (*(ushort *)(param_2 + 6) < 2) {
        return -1;
      }
      **(undefined2 **)(param_1 + 0x30) = 0;
      iVar2 = 2;
LAB_080254aa:
      iVar4 = -1;
      if (false) {
        iVar3 = param_1 + 0x6c;
      }
      else {
        iVar3 = param_1 + 0x34;
      }
      if (*(int *)(iVar3 + 4) != 1) {
        *(undefined4 *)(iVar3 + 0x14) = *(undefined4 *)(param_1 + 0x30);
        *(int *)(iVar3 + 0x18) = iVar2;
        iVar4 = FUN_08030b04();
        if (-1 < iVar4) {
          return iVar2;
        }
      }
      return iVar4;
    }
    if (bVar1 == 1) {
      return -1;
    }
  }
  else {
    if (bVar1 == 10) {
      if (iVar2 == 0) {
        return -1;
      }
      if (*(short *)(param_2 + 6) == 0) {
        return -1;
      }
      **(undefined1 **)(param_1 + 0x30) = *(undefined1 *)(iVar2 + 0x14);
      iVar2 = 1;
      goto LAB_080254aa;
    }
    if (bVar1 == 0xb) {
      if (iVar4 == 0) {
        return -1;
      }
      iVar2 = FUN_0801a6a8(iVar4,*(ushort *)(param_2 + 4) & 0xff,*(undefined1 *)(param_2 + 2));
      return iVar2;
    }
  }
  if ((iVar2 != 0) && (*(code **)(iVar2 + 0x10) != (code *)0x0)) {
                    /* WARNING: Could not recover jumptable at 0x0802547c. Too many branches */
                    /* WARNING: Treating indirect jump as call */
    iVar2 = (**(code **)(iVar2 + 0x10))();
    return iVar2;
  }
  return -1;
}



/* WARNING: Globals starting with '_' overlap smaller symbols at the same address */

/* 0x080254d4 */
void FUN_080254d4(param_1)
int param_1;
{
  int iVar1;
  int iVar2;
  
  if ((_DAT_87c74500 != 1) && (_DAT_87c74538 != 1)) {
    iVar1 = 0;
    do {
      if (*(int *)(iVar1 * 0x38 + -0x7838bae8) == 0) {
        do {
          iVar2 = FUN_08030b04(iVar1 * 0x38 + -0x7838bb04);
          if (iVar2 == 0) {
            return;
          }
          FUN_0802fc0c(&DAT_0804dde4);
        } while (*(char *)(param_1 + 0x29) != '\0');
        return;
      }
      iVar1 = iVar1 + 1;
    } while (iVar1 < 2);
  }
  return;
}




/* 0x08025618 */
undefined4 FUN_08025618(param_1)
undefined4 * param_1;
{
  int iVar1;
  
  iVar1 = 0;
  do {
    if ((&DAT_87c1ad00)[iVar1 * 0x87] == 0) {
      if (iVar1 < 4) goto LAB_08025642;
      break;
    }
    iVar1 = iVar1 + 1;
  } while (iVar1 < 4);
  FUN_08015a50();
LAB_08025642:
  (&DAT_87c1ad00)[iVar1 * 0x87] = 1;
  if (param_1 != (undefined4 *)0x0) {
    *param_1 = &DAT_87c1aae8 + iVar1 * 0x21c;
  }
  return 0;
}




/* 0x08025660 */
undefined4 FUN_08025660(param_1)
undefined4 * param_1;
{
  int iVar1;
  
  iVar1 = 0;
  do {
    if ((&DAT_87c1a6dc)[iVar1 * 0x102] == 0) {
      if (iVar1 < 2) goto LAB_08025686;
      break;
    }
    iVar1 = iVar1 + 1;
  } while (iVar1 < 2);
  FUN_08015a50();
LAB_08025686:
  (&DAT_87c1a6dc)[iVar1 * 0x102] = 1;
  if (param_1 != (undefined4 *)0x0) {
    *param_1 = &UNK_87c1a2d8 + iVar1 * 0x408;
  }
  return 0;
}




/* 0x080256a0 */
undefined4 FUN_080256a0(param_1)
undefined4 * param_1;
{
  int iVar1;
  
  iVar1 = 0;
  do {
    if ((&DAT_87c19cc8)[iVar1 * 0x81] == 0) {
      if (iVar1 < 4) goto LAB_080256c6;
      break;
    }
    iVar1 = iVar1 + 1;
  } while (iVar1 < 4);
  FUN_08015a50();
LAB_080256c6:
  (&DAT_87c19cc8)[iVar1 * 0x81] = 1;
  if (param_1 != (undefined4 *)0x0) {
    *param_1 = &UNK_87c19ac8 + iVar1 * 0x204;
  }
  return 0;
}




/* 0x08025740 */
uint FUN_08025740(param_1)
byte * param_1;
{
  uint uVar1;
  
  if (param_1[1] == 0x3a) {
    uVar1 = *param_1 - 0x41;
    if (uVar1 < 0x1a) {
      return uVar1;
    }
    uVar1 = *param_1 - 0x61;
    if (uVar1 < 0x1a) {
      return uVar1;
    }
  }
  FUN_08015a50();
  return 0;
}




/* 0x08025760 */
void FUN_08025760(param_1)
int param_1;
{
  if ((param_1 == 0) || (*(int *)(param_1 + 0x218) == 0)) {
    FUN_08015a50();
  }
  *(undefined4 *)(param_1 + 0x218) = 0;
  return;
}




/* 0x08025778 */
void FUN_08025778(param_1)
int param_1;
{
  if ((param_1 == 0) || (*(int *)(param_1 + 0x404) == 0)) {
    FUN_08015a50();
  }
  *(undefined4 *)(param_1 + 0x404) = 0;
  return;
}




/* 0x08025790 */
void FUN_08025790(param_1)
int param_1;
{
  if ((param_1 == 0) || (*(int *)(param_1 + 0x200) == 0)) {
    FUN_08015a50();
  }
  *(undefined4 *)(param_1 + 0x200) = 0;
  return;
}




/* 0x080257ee */
undefined4 FUN_080257ee(param_1)
int param_1;
{
  *(undefined1 *)(param_1 + 0x861c) = 0;
  *(undefined4 *)(param_1 + 0x8610) = 0;
  return 0;
}




/* 0x08025820 */
undefined4 FUN_08025820(param_1)
undefined4 param_1;
{
  undefined4 uVar1;
  
  uVar1 = 0xfffffffb;
  if (true) {
    switch(param_1) {
    case 0:
      uVar1 = 0;
      break;
    case 1:
    case 2:
    case 0x10:
      return 0xfffffc05;
    case 3:
      return 0xfffffc04;
    case 5:
      return 0xfffffc16;
    case 6:
      return 0xfffffc12;
    case 7:
    case 0x21:
      return 0xfffffffa;
    case 8:
      return 0xfffffc0f;
    case 9:
      return 0xffff8000;
    case 10:
    case 0xf:
    case 0x13:
    case 0x14:
    case 0x17:
    case 0x18:
    case 0x19:
    case 0x1a:
    case 0x1b:
    case 0x1c:
    case 0x1f:
    case 0x20:
      return 0xffff7ffe;
    case 0xb:
      return 0xfffffbfb;
    case 0xc:
      return 0xfffffbfe;
    case 0xd:
      return 0xfffffc0b;
    case 0xe:
      return 0xfffffbf1;
    case 0x16:
    case 0x1d:
      return 0xfffffc08;
    case 0x1e:
      return 0xfffffbf4;
    case 0x25:
      return 0xfffffc03;
    }
  }
  return uVar1;
}




