...
impl<'a, H: SdccHw> Console<'a, H> {
    ...
    pub fn run(&mut self) -> ! {
        let banner = "=== MSM8909 Diag Console (Rust) ===\r\n\
                       ...
                       \x20 G               read GPT table\r\n\
                       \x20 F NAME|SEC CNT  verified flash\r\n\
                       \x20 R ADDR LEN     raw memory dump\r\n\
                       > ";
        ...
        loop {
            ...
            match cmd_byte {
                ...
                b'G' => self.cmd_gpt(),
                b'F' => self.cmd_flash(&args[..args_len]),
                b'R' => self.cmd_raw_read(&args[..args_len]),
                _ => {
    ...
    fn cmd_raw_read(&mut self, args: &[u8]) {
        let p1 = skip_ws(args);
        let addr = parse_hex(p1);
        let p2 = skip_ws(skip_nonws(p1));
        let len = parse_hex(p2) as usize;

        let mut f = Fmt::new(&mut self.resp);

        if len == 0 || len > BULK_BUF_SIZE {
            f.str("ERR invalid len (max ");
            f.hex32(BULK_BUF_SIZE as u32);
            f.str(")\n");
            f.flush();
            return;
        }

        // Send READY with address and length
        f.str("READY ");
        f.hex32(addr);
        f.str(" ");
        f.hex32(len as u32);
        ...
        // Copy memory into BULK_BUF, then send over USB
        let bulk = unsafe { &mut BULK_BUF[..len] };
        let src = addr as *const u8;
        for i in 0..len {
            bulk[i] = unsafe { core::ptr::read_volatile(src.add(i)) };
        }

        // SHA256 of the data
        let hash = sha256::sha256(bulk);

        // Send raw binary data
        let mut sent = 0;
        while sent < len {
            let chunk = if len - sent > 4096 { 4096 } else { len - sent };
            usb::write(&bulk[sent..sent + chunk]);
            sent += chunk;
        }

        // Send SHA256 as text on a new line
        f.str("SHA256 ");
        for &b in &hash {
            f.hex8(b);
        }
    ...
    fn cmd_gpt(&mut self) {
        ...
        let buf = unsafe { &mut SECTOR_BUF };
        match gpt::read_gpt(self.emmc, buf) {
            Ok((entries, count)) => {
                self.gpt_entries = entries;
                self.gpt_count = count as u8;

                for i in 0..count {
                    let e = &entries[i];
                    // name start_hex count_hex [attrs_hex if non-zero]
                    for j in 0..e.name_len as usize {
                        f.buf[f.pos] = e.name[j];
                        f.pos += 1;
                    }
                    f.str(" ");
                    f.hex32(e.start);
                    ...
                    if e.attrs != 0 {
                        f.str(" ");
                        f.hex32((e.attrs >> 32) as u32);
                        f.hex32(e.attrs as u32);
                    }
                    f.str("\n");
                    f.flush_if_full();
                }
            ...
            Err(_) => {
                f.str("ERR GPT read failed\n");
            }
        }
        f.flush();
    }

    fn cmd_flash(&mut self, args: &[u8]) {
        let p1 = skip_ws(args);
        ...
        // Parse: either "sector count" (two hex args) or "name" (GPT lookup).
        // Manual mode requires a space between two hex numbers.
        // Do this before creating Fmt to avoid borrow conflict with self.
        let (sector, count) = if has_two_tokens(p1) && is_hex_digit(first_byte(p1)) {
            let sector = parse_hex(p1);
            let p2 = skip_ws(skip_nonws(p1));
            let count = parse_hex(p2);
            (sector, count)
        } else {
            let name_end = find_ws_or_end(p1);
            let name = &p1[..name_end];
            match self.find_gpt(name) {
                Some((start, sectors)) => (start, sectors),
                None => {
                    let mut f = Fmt::new(&mut self.resp);
                    f.str("ERR partition not found (run G first)\n");
                    f.flush();
                    return;
                }
            }
        ...
        let mut confirm_buf = [0u8; CMD_BUF_SIZE];
        let n = usb::read(&mut confirm_buf);
        if n <= 0 || !starts_with(&confirm_buf[..n as usize], b"CONFIRM") {
            f.str("ABORTED\n");
            f.flush();
            return;
        }

        // Write to eMMC in 64-sector chunks
        ...
        // Read back into BULK_BUF and hash
        let mut read_back: u32 = 0;
        while read_back < count {
            let chunk = if count - read_back > 64 { 64 } else { count - read_back };
            let offset = (read_back as usize) * SECTOR_SIZE;
            let buf_slice = &mut bulk[offset..offset + chunk as usize * SECTOR_SIZE];
            match self.emmc.read(sector + read_back, buf_slice) {
                Ok(()) => {}
                Err(_) => {
                    f.str("ERR readback failed at sector ");
                    f.hex32(sector + read_back);
                    f.str("\n");
                    f.flush();
                    return;
                }
        ...
        let rb_hash = sha256::sha256(bulk);
        f.str("DONE ");
        for &b in &rb_hash {
            f.hex8(b);
        }
    ...
    fn find_gpt(&self, name: &[u8]) -> Option<(u32, u32)> {
        for i in 0..self.gpt_count as usize {
            if self.gpt_entries[i].name_eq(name) {
                return Some((self.gpt_entries[i].start, self.gpt_entries[i].sectors));
            }
        }
        None
    }
...
fn has_two_tokens(s: &[u8]) -> bool {
    ...
    // Must have a second token
    i < s.len() && s[i] != b'\r' && s[i] != b'\n'
}
